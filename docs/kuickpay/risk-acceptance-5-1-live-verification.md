...
- **AC2** — A real **`confirmed_unposted → posted`** round-trip on a genuinely bank-paid voucher: real
  ...
  **transaction #267082** created and applied to the invoice, and **idempotency** proven (no duplicate, no
...
- **Bugs found and fixed live** (merged to `main`, commit `2997f481`): missing
  ...
  Plus the restored admin `unauthorized.pdt` (blank → clean message on denial) and the staff-group
...
3. **Real `InsertVoucher` (voucher creation)** — proven in practice (the account owner created the paid
   voucher #2 via the real checkout) but not by a repeatable automated test in this effort.
4. **Independent code review of the money-path validator fixes** — the three fixes are unit-green and
   live-proven but warrant a formal second-reviewer pass (payment-validation code).
5. **Pre-existing gateway baseline red** — the `bill-payment-inquiry-empty-currency` fail-closed-contract
   test remains failing (1 test); unrelated to the behavior verified here.
6. **Restored core view** `app/views/admin/paradigm/unauthorized.pdt` is a local patch to the Blesta core
   view tree (a missing-file restoration); a future Blesta upgrade may supersede it.
7. **Operational step**: after install/upgrade, the plugin's permissions must be granted to each staff group
...
- **Project Lead (Israr):** ____________________  **Date:** __________
