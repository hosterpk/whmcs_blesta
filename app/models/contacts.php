...
class Contacts extends AppModel
{
    ...
    public function add(array $vars)
    {
        ...
        if ($this->validateContact($vars, false, true)) {
            ...
            // Log that the contact was added
    ...
    private function getReadOnlyFieldRules(array $vars, $edit = false, $staff_id = null)
    {
        ...
        if (($staff = $this->Staff->get($staff_id))) {
            return [];
        }
    ...
    public function edit($contact_id, array $vars)
    {
        ...
        if ($this->validateContact($vars, true, true)) {
            ...
            if (($client = $this->Clients->get($old_contact['client_id'], true))) {
                ...
                if ($vars['verify'] && isset($vars['email'])) {
                    // Add email verification
            ...
            // Update contact
            $r = $this->Record->where('id', '=', $contact_id)->update('contacts', $vars, $fields);
            ...
            // Log that the contact was updated
            $requestor = $this->getFromContainer('requestor');
            $this->logger->info(
                'Updated Contact',
                array_merge(
                    ['contact_id' => $contact_id, 'by_user_id' => $requestor->user_id],
                    array_intersect_key($vars, array_flip($fields))
                )
            );
    ...
    public function get($contact_id)
    {
        ...
        $contact = $this->Record->select($fields)
            ->from('contacts')
        ...
        if (isset($contact->state)) {
            $state = $this->Record->select(['states.*', 'countries.format', 'countries.name' => 'country'])
                ...
                ->where('countries.alpha2', '=', $contact->country)
                ->where('states.code', '=', $contact->state)
                ->fetch();

            if (($state->format ?? '') == 'name') {
                $contact->country_name = $state->country ?? $contact->country;
                $contact->state_name = $state->name ?? $contact->state;
            }
        }
    ...
    public function setPermissions($contact_id, array $vars)
    {
        ...
        if ($this->Input->validates($data)) {
            ...
            // Log that the contact permissions was updated
            $requestor = $this->getFromContainer('requestor');
            $this->logger->info(
                'Updated Contact Permissions',
                array_merge($data, $vars, ['by_user_id' => $requestor->user_id])
            );
        }
    }
    ...
    public function getPermissionOptions($company_id)
    {
        ...
        $options = [
            ...
            'client_quotations' => $this->_('Contacts.getPermissionOptions.client_quotations'),
        ...
        if ($multiple) {
            if (!empty($required_types)) {
                ...
                foreach ($required_types as $required_type) {
                    $rules['numbers'][$required_type . '_required'] = [
                        'if_set' => $edit,
                        'rule' => function ($numbers) use ($required_type) {
                            $valid = false;
                            foreach ($numbers ?? [] as $number) {
                                if (($number['type'] ?? '') == $required_type && !empty($number['number'])) {
                                    $valid = true;
                                    break;
                                }
                            }
    ...
    public function validateContact(array $vars, $edit = false, $validate_client = false)
    {
        ...
        // Validate the provided email is not blacklisted
        if (empty($this->Session->read('blesta_staff_id'))) {
            $rules['email']['blacklist'] = [
                'if_set' => true,
        ...
        if ($edit) {
            ...
            $rules['contact_type']['inv_address_to'] = [
                'if_set' => true,
                'rule' => [[$this, 'validateReceivesInvoices'], ($vars['contact_id'] ?? null)],
                'negate' => true,
                'message' => $this->_('Contacts.!error.contact_type.inv_address_to')
            ];
