...
class PackageOptionGroups extends AppModel
{
     ...
     *
     * @param int $option_group_id The ID of the option group to export
     * @return mixed A JSON-encoded object representing a option group, or void on error
     */
    public function export($option_group_id)
    {
        if (!isset($this->PackageOptions)) {
            Loader::loadModels($this, ['PackageOptions']);
        }

        if (!isset($this->PackageOptionConditions)) {
            Loader::loadModels($this, ['PackageOptionConditions']);
        }

        if (!isset($this->PackageOptionConditionSets)) {
            Loader::loadModels($this, ['PackageOptionConditionSets']);
        }

        // Get option group
        $option_group = $this->get($option_group_id);
        if (!$option_group) {
            return false;
        }
        ...
        $remove_fields = [
            'company_id', 'option_id', 'packages', 'pricing_id', 'option_value_id', 'option_group_id'
        ];
        foreach ($remove_fields as $field) {
            if (isset($option_group->{$field})) {
                unset($option_group->{$field});
            }
        }

        // Get group options
        $option_group->options = $this->PackageOptions->getByPackageOptionGroupId($option_group_id);
        ...
        // Get condition sets
        $option_group->condition_sets = $this->PackageOptionConditionSets->getAll(['option_group_id' => $option_group_id]);
        foreach ($option_group->condition_sets as &$option_set) {
            $option_set->conditions = $this->PackageOptionConditions->getAll(['condition_set_id' => $option_set->id]);
            foreach ($remove_fields as $field) {
                if (isset($option_set->{$field})) {
                    unset($option_set->{$field});
                }
                if (isset($option_set->option->{$field})) {
                    unset($option_set->option->{$field});
                }
            }
        }
     ...
     * @return bool True if the file has been exported successfully, false otherwise
     */
    public function import(array $vars, $company_id = null)
    {
        if (!isset($this->PackageOptions)) {
            Loader::loadModels($this, ['PackageOptions']);
        }
        ...
        $map['groups'][$vars['id']] = $this->add($vars);
        ...
        foreach ($vars['options'] as $option) {
            ...
            $option['groups'] = [$map['groups'][$vars['id']]];

            $map['options'][$option['id']] = $this->PackageOptions->add($option);
            ...
            // Map recently added values
            foreach ($option['values'] as $value) {
                $option_value = $this->Record->select()
                    ->from('package_option_values')
                    ->where('package_option_values.option_id', '=', $map['options'][$option['id']])
                    ->where('package_option_values.name', '=', $value['name'])
                    ->where('package_option_values.value', '=', $value['value'])
                    ->where('package_option_values.status', '=', $value['status'])
                    ->where('package_option_values.order', '=', $value['order'])
                    ->fetch();
                $map['values'][$value['id']] = $option_value->id;
            }
        }

        // Add condition sets
        foreach ($vars['condition_sets'] as $condition_set) {
            $formatted_condition_set = [
                'option_group_id' => $map['groups'][$vars['id']],
                'option_id' => $map['options'][$condition_set['option']['id']],
            ];
            foreach ($condition_set['option_value_ids'] as $option_value_id) {
                if (!isset($map['values'][$option_value_id])) {
                    continue;
                }

                $formatted_condition_set['option_value_ids'][] = $map['values'][$option_value_id];
            }

            $map['condition_sets'][$condition_set['id']] = $this->PackageOptionConditionSets->add(
                $formatted_condition_set
            );
            ...
            foreach ($condition_set['conditions'] as $condition) {
                $formatted_condition = [
                    'condition_set_id' => $map['condition_sets'][$condition_set['id']],
                    'trigger_option_id' => $map['options'][$condition['trigger_option_id']],
                    'operator' => $condition['operator'],
                    'value' => $condition['value'],
                    'value_id' => $condition['value_id']
                ];
                $map['conditions'][$condition['id']] = $this->PackageOptionConditions->add(
                    $formatted_condition
                );
