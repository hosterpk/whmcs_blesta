...
class Accounts extends AppModel
{
    ...
    public function getAllCc($contact_id)
    {
        ...
        $accounts = $this->Record->select(['accounts_cc.*', 'gateways.name' => 'gateway_name', 'contacts.client_id' => 'client_id'])
            ->select(["'cc'" => 'account_type'], false)
            ->from('accounts_cc')
            ->leftJoin('gateways', 'gateways.id', '=', 'accounts_cc.gateway_id', false)
            ->leftJoin('contacts', 'contacts.id', '=', 'accounts_cc.contact_id', false)
            ->where('accounts_cc.contact_id', '=', $contact_id)
            ->where('status', '=', 'active')
            ->fetchAll();
        ...
        // Decrypt fields
        if ($accounts) {
            $default_gateways_setting = $this->Companies->getSetting(
                Configure::get('Blesta.company_id'),
                'default_merchant_gateway'
            );
            $default_gateways_by_currency = json_decode($default_gateways_setting->value ?? '', true);
            foreach ($default_gateways_by_currency as $currency => $currency_gateway_id) {
                $gateway = $this->GatewayManager->get($currency_gateway_id);
                $default_gateways_by_currency[$currency] = ($gateway->name ?? null) . '*';
            }
    ...
    {
        $accounts = $this->Record->select(['accounts_cc.*', 'gateways.name' => 'gateway_name'])
            ->select(["'cc'" => 'account_type'], false)
            ->from('accounts_cc')
        ...
        // Decrypt fields
        if ($accounts) {
            // Get default gateway settings for NULL gateway_id accounts
            if (!isset($this->Companies)) {
                Loader::loadModels($this, ['Companies']);
            }
            if (!isset($this->GatewayManager)) {
            ...
            if (is_array($default_gateways_by_currency)) {
                foreach ($default_gateways_by_currency as $currency => $currency_gateway_id) {
                    $gateway = $this->GatewayManager->get($currency_gateway_id);
                    $default_gateways_by_currency[$currency] = ($gateway->name ?? null) . '*';
                }
            } else {
                $default_gateways_by_currency = [];
            }
    ...
    {
        $accounts = $this->Record->select(['accounts_ach.*', 'gateways.name' => 'gateway_name'])
            ->select(["'ach'" => 'account_type'], false)
            ->from('accounts_ach')
            ->innerJoin('gateways', 'gateways.id', '=', 'accounts_ach.gateway_id', false)
            ->where('accounts_ach.contact_id', '=', $contact_id);
    ...
    public function getAllAchByClient($client_id, $unverified = false)
    {
        $accounts = $this->Record->select(['accounts_ach.*', 'gateways.name' => 'gateway_name'])
            ->select(["'ach'" => 'account_type'], false)
            ->leftJoin('gateways', 'gateways.id', '=', 'accounts_ach.gateway_id', false)
            ->from('accounts_ach')
    ...
    public function addAch(array $vars)
    {
        ...
        if ($this->verifyAch($vars)) {
            ...
            if ($response === false) {
                ...
                // Set gateway_id to the current default merchant gateway since we're storing locally
                if (!isset($vars['gateway_id']) || $vars['gateway_id'] === null) {
                    if (!isset($this->Clients)) {
                        Loader::loadModels($this, ['Clients']);
                    }
                    ...
                    // Get the client's default currency
                    $contact = $this->Contacts->get($vars['contact_id']);
                    if ($contact) {
                        $currency = $vars['currency'] ?? null;
                        if (!$currency) {
                            $currency_setting = $this->Clients->getSetting($contact->client_id, 'default_currency');
                            $currency = $currency_setting->value ?? 'USD';
                        }

                        // Get the default merchant gateway for this currency
                        $gateway = $this->GatewayManager->getInstalledMerchant(
                            Configure::get('Blesta.company_id'),
                            $currency
                        );
                        if ($gateway) {
                            $vars['gateway_id'] = $gateway->id;
                        }
                    }
                }
            }
    ...
    /**
     * Migrates payment accounts with NULL gateway_id to a specific gateway
     *
     * This is called when changing the default merchant gateway to ensure
     * accounts remain associated with their original gateway.
     *
     * @param string $currency The currency code (e.g., 'USD')
     * @param int $gateway_id The gateway ID to set for NULL accounts
     */
    public function migrateNullGatewayAccounts($currency, $gateway_id)
    {
        // Update NULL gateway_id CC accounts for clients with this default currency
        $this->Record->query(
            "UPDATE accounts_cc
            INNER JOIN contacts ON accounts_cc.contact_id = contacts.id
            INNER JOIN client_settings ON contacts.client_id = client_settings.client_id
            SET accounts_cc.gateway_id = ?
            WHERE accounts_cc.gateway_id IS NULL
            AND client_settings.key = 'default_currency'
            AND client_settings.value = ?",
            [$gateway_id, $currency]
        );

        // Update NULL gateway_id ACH accounts for clients with this default currency
        $this->Record->query(
            "UPDATE accounts_ach
            INNER JOIN contacts ON accounts_ach.contact_id = contacts.id
            INNER JOIN client_settings ON contacts.client_id = client_settings.client_id
            SET accounts_ach.gateway_id = ?
            WHERE accounts_ach.gateway_id IS NULL
            AND client_settings.key = 'default_currency'
            AND client_settings.value = ?",
            [$gateway_id, $currency]
        );
