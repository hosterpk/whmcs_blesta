...
final class LicenseManager
{
    ...
    {
        foreach ($this->getFileSignatures(array_keys($signatures)) as $file => $sig) {
            if (is_array($signatures[$file]) && !in_array($sig, $signatures[$file])) {
                return false;
            } elseif (is_string($signatures[$file]) && $signatures[$file] != $sig) {
                return false;
            }
        }
        return true;
    ...
    private function getServerInfo()
    {
        ...
        } elseif (!empty(gethostname())) {
            // Usually works on CLI environments
