...
class ManagedAccounts extends AppModel
{
    ...
    public function invite(int $client_id, string $email, array $permissions)
    {
            ...
            'email' => [
                'invitation' => [
                    'rule' => function ($email) use ($client_id) {
                        // Check if a previous invitation doesn't exist for this client
                ...
                'exists' => [
                    'rule' => function ($email) {
                        // Check if a primary contact exists with this email address
                    ...
                    'message' => $this->_('ManagedAccounts.!error.email.exists')
                ]
            ],
            'permissions' => [
                'empty' => [
        ...
        if ($this->Input->validates($data)) {
            ...
            // Log that the manager was invited
            $requestor = $this->getFromContainer('requestor');
            $this->logger->info(
                'Invited Manager',
                array_merge($data, ['by_user_id' => $requestor->user_id])
            );
    ...
    public function setPermissions(int $contact_id, int $client_id, array $vars)
    {
        ...
        if ($this->Input->validates($data)) {
            ...
            // Log that the manager permissions was updated
            $requestor = $this->getFromContainer('requestor');
            $this->logger->info(
