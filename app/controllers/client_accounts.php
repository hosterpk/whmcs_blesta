...
class ClientAccounts extends ClientController
{
    ...
    public function creditHandling()
    {
        ...
        // Only show if credit is enabled
        if (($this->client->settings['payment_credit_enabled'] ?? '1') == '0') {
            $this->redirect($this->base_uri . 'accounts/');
        }

        $vars = new stdClass();

        $this->setMessage('info', Language::_('ClientAccounts.credithandling.text_description', true));

        // Update credit threshold settings
        if (!empty($this->post)) {
                ...
                foreach ($this->post['credit_thresholds'] as $currency => $threshold) {
                    if ($threshold !== '') {
                        ...
                        if ($normalized !== null && is_numeric($normalized) && (float)$normalized > 0) {
                            $credit_balance_thresholds[$currency] = number_format((float)$normalized, 4, '.', '');
                        }
                    }
                }
            }
            ...
            $this->Clients->setSetting(
                $this->client->id,
                'credit_balance_thresholds',
                json_encode($credit_balance_thresholds)
            );
            ...
            if (($errors = $this->Clients->errors())) {
                ...
                $this->setMessage('message', Language::_('ClientAccounts.!success.credit_handling_updated', true));
                $vars->credit_thresholds = $credit_balance_thresholds;
            }
        } else {
            // Load current settings
            $credit_thresholds = [];
            if (isset($this->client->settings['credit_balance_thresholds'])
                && !empty($this->client->settings['credit_balance_thresholds'])) {
                $credit_thresholds = json_decode($this->client->settings['credit_balance_thresholds'], true);
            }
            $vars->credit_thresholds = $credit_thresholds;
        }
