...
use Blesta\Core\Util\Filters\CouponFilters;
 ...
 */
class AdminCompanyBilling extends AdminController
{
    /**
     * Pre-action
     */
    ...
    public function invoices()
    {
        ...
        if (!empty($this->post)) {
            ...
            foreach ($service_attempt_types as $service_attempt_type => $attempt_label) {
                $service_attempt_fields[] = 'service_' . $service_attempt_type . '_attempts';
                $service_attempt_fields[] = 'first_' . $service_attempt_type . '_attempt_threshold';
                $service_attempt_fields[] = 'first_' . $service_attempt_type . '_attempt_spacing';
                $service_attempt_fields[] = 'second_' . $service_attempt_type . '_attempt_threshold';
                $service_attempt_fields[] = 'second_' . $service_attempt_type . '_attempt_spacing';
            }

            $fields = array_merge(['inv_days_before_renewal', 'autodebit_days_before_due',
                'suspend_services_days_after_due', 'autodebit_attempts', 'clients_cancel_options',
                'cancel_service_changes_days', 'apply_inv_late_fees'], $checkboxes, $service_attempt_fields);
        ...
        $autodebit_attempts = [];
        $service_attempts = [];
        $service_attempt_spacing = [Language::_('AdminCompanyBilling.invoices.text_none', true)];
        ...
        for ($i = 1; $i <= 30; $i++) {
            $service_attempts[$i] = $i;
        }
        for ($i = 1; $i <= 72; $i++) {
            $service_attempt_spacing[$i] = Language::_(
        ...
        $form_fields = [
            ...
            'service_attempt_spacing' => $service_attempt_spacing,
    ...
    public function deliveryMethods()
    {
        ...
        if (!empty($this->post)) {
            ...
            $this->post['delivery_methods'] = base64_encode(json_encode($this->post['delivery_methods']));
            ...
            $fields = ['delivery_methods', 'interfax_username', 'interfax_password',
                ...
                'postalmethods_return_name', 'postalmethods_return_company',
    ...
    public function coupons()
    {
        ...
        // Get all coupons
        $coupons = $this->Coupons->getList($this->company_id, $page, ['code' => 'ASC'], $filters);
        $total_results = $this->Coupons->getListCount($this->company_id, $filters);

        // Set the input field filters for the widget
        $service_filters = new CouponFilters();
        $this->set(
            'filters',
    ...
    {
        $this->uses(['Coupons', 'CouponTerms', 'Currencies', 'Packages', 'PackageOptions', 'PackageOptionGroups']);
        ...
        if (!empty($this->post)) {
            ...
            // Add package option limitations
            if (!$errors && !empty($this->post['package_options'])) {
                if (!isset($this->CouponPackageOptions)) {
                    $this->uses(['CouponPackageOptions']);
                }

                foreach ($this->post['package_options'] as $package_option) {
                    // Skip empty package options
                    if (empty($package_option['option_group_id']) || empty($package_option['option_id'])) {
                        continue;
                    }

                    $option_vars = [
                        'coupon_id' => $coupon_id,
                        'option_group_id' => $package_option['option_group_id'],
                        'option_id' => $package_option['option_id']
                    ];

                    // Handle different option types - let the model handle encoding
                    if (!empty($package_option['option_value_ids'])) {
                        $option_vars['option_value_ids'] = $package_option['option_value_ids'];
                    } elseif (!empty($package_option['min_quantity'])) {
                        $option_vars['min_quantity'] = $package_option['min_quantity'];
                    } elseif (!empty($package_option['regex_pattern'])) {
                        $option_vars['regex_pattern'] = $package_option['regex_pattern'];
                    }

                    $this->CouponPackageOptions->add($option_vars);

                    if (($errors = $this->CouponPackageOptions->errors())) {
                        break;
                    }
                }
            }
        ...
        // Set package option groups for configurable option limitations
        $package_option_groups = $this->Form->collapseObjectArray(
            $this->PackageOptionGroups->getAll($this->company_id, ['hidden' => 1]),
            'name',
            'id'
        );
        $this->set('package_option_groups', ['' => Language::_('AppController.select.please', true)] + $package_option_groups);
    ...
    public function editCoupon()
    {
        ...
        if (!empty($this->post)) {
            ...
            // Update package option limitations
            if (!$errors) {
                if (!isset($this->CouponPackageOptions)) {
                    $this->uses(['CouponPackageOptions']);
                }

                // Delete existing package option limitations
                $this->CouponPackageOptions->delete($coupon->id);

                // Add new package option limitations only if fields are not empty
                if (!empty($this->post['package_options'])) {
                    foreach ($this->post['package_options'] as $package_option) {
                        // Skip empty package options
                        if (empty($package_option['option_group_id']) || empty($package_option['option_id'])) {
                            continue;
            ...
            if ($errors) {
                ...
                $this->redirect($this->base_uri . 'settings/company/billing/editcoupon/' . $coupon->id . '/');
            }
        }
        ...
        if (empty($vars)) {
            ...
            $vars->package_options = array_map(function($value) {
                    $converted_value = (array)$value;
                    $converted_value['package_options'] = $this->getPackageOptions($converted_value['option_group_id']);
        ...
        $package_option_groups = ['' => Language::_('AdminCompanyBilling.editcoupon.text_none', true)] + $package_option_groups;
     ...
     */
    public function getPackageOptions($default_group_id = null)
    {
        $this->uses(['PackageOptions']);

        // Check if this was called internally or over ajax
        if (!$this->isAjax() && !$default_group_id) {
            $this->redirect($this->base_uri . 'settings/company/billing/coupons/');
        }
        $group_id = $default_group_id ?? (isset($this->get[0]) ? (int)$this->get[0] : null);
        $options = [];

        if ($group_id) {
            $package_options = $this->PackageOptions->getAll($this->company_id, ['hidden' => 1]);
            $group_options = [];

            // Filter options that belong to the specified group
            foreach ($package_options as $option) {
                $option_groups = $this->PackageOptions->getGroups($option->id);
            ...
            $options = $this->Form->collapseObjectArray($group_options, 'name', 'id');
        }
     ...
     */
    public function getPackageOptionDetails()
    {
        $this->uses(['PackageOptions']);
        ...
        $option_id = isset($this->get[0]) ? (int)$this->get[0] : null;
        $option_details = ['type' => '', 'values' => []];

        if ($option_id) {
            $option = $this->PackageOptions->get($option_id);
            if ($option) {
                $option_details['type'] = $option->type;
                $option_details['name'] = $option->name;

                if (in_array($option->type, ['radio', 'select', 'checkbox'])) {
                    $option_details['values'] = $this->Form->collapseObjectArray($option->values, 'name', 'id');
                }
            }
        }

        $this->outputAsJson($option_details);
        return false;
    }
     ...
     */
    public function creditHandling()
    {
        $this->uses(['Currencies']);
        ...
        if (!empty($this->post)) {
            ...
            // Save the settings (validation happens in the model)
            $fields = ['payment_credit_enabled', 'payment_credit_limits'];
            $this->Companies->setSettings($this->company_id, $this->post, $fields);
            ...
            if (($errors = $this->Companies->errors())) {
                ...
                // Success
                $this->flashMessage('message', Language::_('AdminCompanyBilling.!success.credithandling_updated', true));
                $this->redirect($this->base_uri . 'settings/company/billing/credithandling/');
            }
        }

        $credit_limits = ($vars['payment_credit_limits'] ?? '');
        if (is_string($credit_limits)) {
            $credit_limits = $this->SettingsProcessor->decodeJsonSetting($credit_limits);
        }

        $currencies = $this->Currencies->getAll($this->company_id);
        $credit_form = $this->partial(
            'admin_company_billing_credithandling_form',
            [
                'vars' => $vars,
        ...
        $this->set('credit_form', $credit_form);
    }
}
