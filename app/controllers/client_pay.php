...
class ClientPay extends AppController
{
    ...
    public function index()
    {
        ...
        if (!empty($this->post)) {
            ...
            $invoice_ids = ($this->post['invoice_id'] ?? null);
            ...
            $errors = [];
            $credit = $this->CurrencyFormat->cast(($this->post['credit'] ?? ''), $currency->code);
            if ($credit < 0) {
                $errors = [['invalid_credit' => Language::_('ClientPay.!error.invalid_amount', true)]];
            }

            // Validate credit amount against configured limits
            if ($credit > 0 && isset($client_settings['payment_credit_limits'])) {
                $credit_limits = json_decode($client_settings['payment_credit_limits'], true);
                if (isset($credit_limits[$currency->code])) {
                    $limits = $credit_limits[$currency->code];

                    // Check minimum limit
                    if (isset($limits['min']) && $limits['min'] !== null && $credit < $limits['min']) {
                        $errors[] = ['credit_minimum' => Language::_('ClientPay.!error.credit_minimum', true, $this->CurrencyFormat->format($limits['min'], $currency->code))];
                    }

                    // Check maximum limit (accounting for existing credit balance)
                    if (isset($limits['max']) && $limits['max'] !== null) {
                        $current_credit = $this->Transactions->getTotalCredit($this->client->id, $currency->code);
                        ...
                        if ($credit > $effective_max) {
                            $errors[] = ['credit_maximum' => Language::_('ClientPay.!error.credit_maximum', true, $this->CurrencyFormat->format($effective_max, $currency->code))];
                        }
                    }
                }
            }
            ...
            if (!empty($invoice_ids) || isset($this->post['credit'])) {
                if (($client_settings['payment_credit_enabled'] ?? '1') == 0) {
                    $apply_amounts = [];
                } else {
                    // Normalize credit amount (handle comma-decimal formats)
                    $credit_amount = 0;
                    if (isset($this->post['credit']) && $this->post['credit'] !== '') {
                        $credit_amount = $this->CurrencyFormat->cast($this->post['credit'], $currency->code);
                    }
        ...
        // If display_payments setting is enabled, check if any invoice has payments applied
        // and auto-show the amounts column to clarify partial payments
        if ($toggle_amounts) {
            ...
            if (isset($company_settings['inv_display_payments'])
                ...
                // Check if any invoice has a different due amount than total (indicating payments applied)
                foreach ($invoice_list as $inv) {
        ...
        if (isset($client_settings['payment_credit_limits']) && $currency) {
            $credit_limits = json_decode($client_settings['payment_credit_limits'], true);
            if (isset($credit_limits[$currency->code])) {
                $limits = $credit_limits[$currency->code];
                $min_value = null;
                ...
                if (isset($limits['min']) && $limits['min'] !== null) {
                    ...
                    $credit_min = $this->CurrencyFormat->format($limits['min'], $currency->code);
                ...
                if (isset($limits['max']) && $limits['max'] !== null) {
                    ...
                    $credit_max = $this->CurrencyFormat->format($effective_max, $currency->code);
                    $has_credit_limits = true;
                }
    ...
    public function method()
    {
        ...
        if (isset($this->get[0]) && isset($invoice) && $invoice->client_id == $this->client->id) {
            ...
            if ($invoices && ($client_settings['payment_credit_enabled'] ?? '1') == '0') {
                $invoices['credit'] = 0;
            }
        ...
        $nm_gateways = $this->GatewayManager->getAllInstalledNonmerchant(
            $this->company_id,
            $invoices['currency'],
            $this->client->id
        );
        ...
        if (!empty($this->post)) {
            $errors = [];
            $this->post['pay_with'] = $this->post['pay_with'] ?? null;
        ...
        // Set the total due
        $credit_value = $invoices['credit'] ?? 0;
        // Normalize credit value if it's a string with comma decimal
        if (is_string($credit_value) && strpos($credit_value, ',') !== false) {
            $credit_value = $this->CurrencyFormat->cast($credit_value, $invoices['currency']);
        }
        $total = (float)$credit_value;

        // Calculate the total to pay for each invoice
        foreach ($invoices['amounts'] as $inv) {
            $amount = $inv['amount'] ?? 0;
            // Normalize amount value if it's a string with comma decimal
            if (is_string($amount) && strpos($amount, ',') !== false) {
                $amount = $this->CurrencyFormat->cast($amount, $invoices['currency']);
            }
            $total += (float)$amount;
        }
        ...
        if (!empty($this->post)) {
            ...
            if ($payment['method']['pay_with'] == 'account' || $payment['method']['pay_with'] == 'details') {
                ...
                if (empty($transaction_id)) {
                    ...
                    $hash = $this->Payments->systemHash('t=' . $transaction_id . '|c=' . $this->client->id);
    ...
    private function authorizePayment(array $payment, array $apply_amounts, $total)
    {
        ...
        if ((isset($payment['method']['payment_account'])
            ...
            if ($transaction) {
                ...
                $hash = $this->Payments->systemHash('t=' . $transaction->id . '|c=' . $this->client->id);
