 ...
 */
class AdminCompanyAutomation extends AdminController
{
    /**
     * Pre-action
     */
        ...
        foreach ($vars as $group) {
            foreach ($group['tasks'] as &$task) {
                ...
                if (!empty($task->interval)) {
                    $task->supported_intervals = $this->getIntervals($task->key, $task->task_type);
     ...
     */
    private function getIntervals($task = null, $task_type = null)
    {
        ...
        // Add 1-minute interval for supported system tasks
        if ($task_type == 'system') {
            $intervals = [5 => 5, 10 => 10, 15 => 15, 30 => 30, 45 => 45];
            $tasks = [
                'process_service_changes',
                'process_renewing_services',
                'provision_pending_services',
                'unsuspend_services'
            ];
