...
class Email extends Component
{
    ...
    public function buildTransport($transport, array $vars = [])
    {
        ...
        // Check if Blesta ships a custom transport
        $file = COREDIR . 'Util' . DS . 'Transport' . DS . str_replace("\\", '', $class) . '.php';

        if (file_exists($file)) {
            require_once $file;
            $namespace = "\\Blesta\\Core\\Util\\Transport\\" . $class;
        } else {
            // Fallback to built in Symfony transporter
