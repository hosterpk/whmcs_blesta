 ...
 *
 * @package blesta
 * @subpackage components.upgrades
...
{
    /**
     * @var string The environment where the upgrade is being executed, it could be "install" or "upgrade"
     */
    private $environment = 'upgrade';
     ...
     * @param bool $extensions True to include extensions on the upgrade, false otherwise (default, true)
     */
    public function start($from, $to, $callback = null, $extensions = true)
    {
     ...
     * @param callback $callback The callback to execute after each task in the upgrade process (for each version)
     */
    public function startExtensions($callback = null)
    {
     ...
     */
    private function clearNavigationCache()
    {
    ...
    public function processObject($obj, $callback = null)
    {
        ...
        if (method_exists($obj, 'setEnvironment')) {
            $obj->setEnvironment($this->getEnvironment());
        }
     ...
     *
     * @param string $environment The environment where the upgrade tasks are running, "install" or "upgrade"
     */
    public function setEnvironment(string $environment)
    {
        if (in_array($environment, ['install', 'upgrade'])) {
     ...
     *
     * @return string The environment where the tasks are running
     */
