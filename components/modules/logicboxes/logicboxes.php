...
class Logicboxes extends RegistrarModule
{
    ...
    public function upgrade($current_version)
    {
        ...
        if (version_compare($current_version, '2.13.0', '<')) {
            $services = $this->Record->select('services.*')
                ...
                ->where('modules.class', '=', 'logicboxes')
                ->fetchAll();
            ...
            foreach ($services as $service) {
                $domain = $this->Record->select()
                    ...
                    ->where('service_fields.key', '=', 'domain-name')
                    ->fetch();
                $field = [
    ...
    public function getDomainNameServers($domain, $module_row_id = null)
    {
        ...
        $response = $domains->detailsByName(['domain-name' => $domain, 'options' => 'NsDetails']);
    ...
    public function setDomainNameservers($domain, $module_row_id = null, array $vars = [])
    {
        ...
        $id_response = $domains->orderid(['domain-name' => $domain]);
        $this->processResponse($api, $id_response);
        if ($id_response->status() != 'OK') {
            return false;
        }
        ...
        if (isset($vars['use_module']) && $vars['use_module'] == 'true') {
            if (($package->meta->type ?? 'domain') == 'domain') {
                ...
                if (isset($vars['transfer']) || isset($vars['auth-code'])) {
                    $response = $domains->transfer(
                        array_merge(
                            array_intersect_key($vars, $transfer_fields),
                    ...
                    $response = $domains->register(
                        array_merge(
                            array_intersect_key($vars, $domain_fields),
                            ['domain-name' => $vars['domain']]
                        )
                    );
                }
                ...
                if (isset($vars['configoptions']['dns_management'])) {
                    $api->loadCommand('logicboxes_dns');
                    $dns = new LogicboxesDns($api);

                    $response = $dns->activate(['order-id' => $order_id]);
                    $this->processResponse($api, $response);
                }

                return [
                    ['key' => 'domain', 'value' => $vars['domain'], 'encrypted' => 0],
    ...
    public function editService($package, $service, array $vars = [], $parent_package = null, $parent_service = null)
    {
        ...
        if (isset($vars['order-id'])) {
            ...
            $fields = [
                ['key' => 'order-id', 'value' => $vars['order-id'], 'encrypted' => 0],
        ...
        if (isset($vars['use_module']) && $vars['use_module'] == 'true' && isset($order_id)) {
            ...
            // Handle DNS management via config option
            $dns_management = $this->featureServiceEnabled('dns_management', $service);
            if (!$dns_management && isset($vars['configoptions']['dns_management'])) {
        ...
        return $fields; // All this handled by admin/client tabs instead
    }
    ...
    public function getClientAddFields($package, $vars = null)
    {
        if (($package->meta->type ?? 'domain') == 'domain') {
            ...
            $tld = (property_exists($vars, 'domain') ? $this->getTld($vars->{'domain'}, true) : null);
    ...
    public function getClientServiceTabs($service)
    {
        ...
        if (($package->meta->type ?? 'domain') == 'domain') {
            $tabs =  [
                ...
                'tabClientDnsRecords' => [
                    'name' => Language::_('Logicboxes.tab_dnsrecord.title', true),
                    'icon' => 'fas fa-sitemap'
                ],
                'tabClientSettings' => [
                    'name' => Language::_('Logicboxes.tab_settings.title', true),
                    'icon' => 'fas fa-cog'
                ]
            ];
    ...
    public function getRegistrationDate($service, $format = 'Y-m-d H:i:s')
    {
        ...
        return isset($response->creationtime)
            ? $this->Date->format(
                $format,
    ...
    public function validateConnection($key, $reseller_id, $sandbox)
    {
        ...
        $api->loadCommand('logicboxes_domains');
        $domains = new LogicboxesDomains($api);
        $response = $domains->available(['domain-name' => 'logicboxes', 'tlds' => ['com']]);
        $this->processResponse($api, $response);
