...
class Plesk extends Module
{
    ...
    public function getClientTabs($package)
    {
        return [
            ...
            'tabClientLogin' => Language::_('Plesk.tab_client_login', true)
        ];
    }
    ...
    /**
     * Creates an SSO login URL for accessing the Plesk control panel
     *
     * @param string $username The Plesk username to create session for
     * @param string $user_ip The client's IP address
     * @return mixed The SSO login URL or false on failure
     */
    private function createSsoUrl($username, $user_ip)
    {
        $row = $this->getModuleRow();
        if (!$row) {
            return false;
        ...
        try {
            $api_version = $this->getApiVersion($row->meta->panel_version);
            $server = $api->loadCommand('plesk_server', [$api_version]);
            ...
            $response = $server->createSession($params);
