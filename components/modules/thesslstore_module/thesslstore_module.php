...
class ThesslstoreModule extends Module
{
    ...
    public function validateCredential($api_partner_code, $vars, $api_mode = 'TEST')
    {
        try {
            ...
            $api_auth_token = '';
            if ($api_mode == 'LIVE') {
                $api_partner_code = ($vars['api_partner_code_live'] ?? '');
                $api_auth_token = ($vars['api_auth_token_live'] ?? '');
            } elseif ($api_mode == 'TEST') {
                $api_partner_code = ($vars['api_partner_code_test'] ?? '');
                $api_auth_token = ($vars['api_auth_token_test'] ?? '');
            }
    ...
    private function getCredentialRules(array &$vars)
    {
        ...
        return [
            ...
            'api_auth_token_live' => [
                ...
                'valid' => [
                    'rule' => [[$this, 'validateCredential'], $vars, 'LIVE'],
            ...
            'api_auth_token_test' => [
                ...
                'valid' => [
                    'rule' => [[$this, 'validateCredential'], $vars, 'TEST'],
    ...
    private function getPackageRules(array $vars = null)
    {
        $rules = [
            ...
            'meta[thesslstore_is_scan_product]' => [
                'empty' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
            ...
            'meta[thesslstore_validation_type]' => [
                'empty' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
    ...
    public function manageModule($module, array &$vars)
    {
        ...
        if ($credential_added == false) {
            $link_buttons = [
                ['name' => Language::_('ThesslstoreModule.add_credential_row', true), 'attributes' => ['href' => $this->base_uri . 'settings/company/modules/addrow/' . $module->id . '?scr=addcredential']]
            ];
        }
    ...
    public function manageAddRow(array &$vars)
    {
        ...
        if ($scr == 'addcredential') {
           ...
           //Load Packages Model
            Loader::loadModels($this, ['Packages','PackageOptions']);

            $package_data = [];
            $packages = $this->Packages->getAll($company_id, $order = ['id' => 'ASC'], $status = 'active');
            foreach ($packages as $pack) {
                ...
                if (isset($package->meta->thesslstore_product_code)) {
                    ...
                    $package_data[$pack->id]['group_name'] = $package->groups[0]->name ?? '';
            ...
            $delimiter = ',';
            $enclosure = '"';
            $heading = ['Date', 'TheSSLStore Order ID','Vendor ID', 'Product Name', 'Common Name','Issued Date', 'Expire Date','Status','Action', 'Replace By Date'];
        ...
        if ($scr == 'addcredential') {
            foreach ($this->getModuleRows() as $row) {
                if (isset($row->meta->api_partner_code_live)) {
                    $this->Input->setErrors(['invalid_action' => ['internal' => Language::_('ThesslstoreModule.!error.credential_already_exist', true)]]);
                    return false;
                }
            }
            $meta_fields = ['thesslstore_reseller_name','api_partner_code_live', 'api_auth_token_live', 'api_partner_code_test',
                'api_auth_token_test', 'api_mode', 'hide_changeapprover_option'];
            $encrypted_fields = ['api_partner_code_live', 'api_auth_token_live', 'api_partner_code_test', 'api_auth_token_test'];
            ...
            $this->Input->setRules($this->getCredentialRules($vars));
            $hide_changeapprover_option = !empty($vars['hide_changeapprover_option']) ? $vars['hide_changeapprover_option'] : 'NO';
            if ($this->Input->validates($vars)) {
                ...
                foreach ($products as $key => $value) {
                    if (!in_array($value->ProductCode, $already_added_packages)) {
                        ...
                        if (($value->IsSanEnable == 'true' && $value->ProductCode != 'quicksslpremiummd') || ($value->isNoOfServerFree == false && $value->isCodeSigning == false && $value->isScanProduct == false)) {
                            ...
                            $optionGroupValue = ['name' => $packageGroupName . '#' . $value->ProductName, 'description' => '', 'company_id' => Configure::get('Blesta.company_id')];
                            ...
                            if ($value->IsSanEnable == 'true' && $value->ProductCode != 'quicksslpremiummd') {
                                ...
                                foreach ($value->PricingInfo as $price_info) {
                                    ...
                                    $sanPricingArray[] = ['term' => $price_info->NumberOfMonths, 'period' => 'month', 'currency' => 'USD', 'price' => $san_price, 'setup_fee' => '', 'cancel_fee' => ''];
                                }


                                $PackageOptions['values'][0] = $value->MinSan != 0 ? ['name' => 'Additional SAN (' . $value->MinSan . ' domains are included by default)', 'value' => '', 'min' => '0', 'max' => $TotalMaxSan, 'step' => 1, 'pricing' => $sanPricingArray] : ['name' => 'Additional SAN', 'value' => '', 'min' => '0', 'max' => $TotalMaxSan, 'step' => 1, 'pricing' => $sanPricingArray];
                            ...
                            if ($value->isNoOfServerFree == false && $value->isCodeSigning == false && $value->isScanProduct == false) {
                                ...
                                foreach ($value->PricingInfo as $price_info) {
                                    ...
                                    $serverPricingArray[] = ['term' => $price_info->NumberOfMonths, 'period' => 'month', 'currency' => 'USD', 'price' => $server_price, 'setup_fee' => '', 'cancel_fee' => ''];
                                }
                                $PackageOptions['values'][0] = ['name' => 'Additional SERVER', 'value' => '', 'min' => '0', 'max' => $TotalMaxServer, 'step' => 1, 'pricing' => $serverPricingArray];
                                $PackageOptions['groups'][0] = $optionGroupId;
                                $PackageOptions['company_id'] = Configure::get('Blesta.company_id');
                                $PackageOptions['editable'] = 0;
                                /* Load the Loader to add options for respective option group */
                        ...
                        $packageArray['pricing'] = [];

                        //Setup Price
                        foreach ($value->PricingInfo as $price_info) {
                            $final_price = $this->setupPrice($price_info->Price, $profitmargin);
                            $packageArray['pricing'][] = ['term' => $price_info->NumberOfMonths, 'period' => 'month', 'currency' => 'USD', 'price' => $final_price, 'setup_fee' => '', 'cancel_fee' => ''];
                        }

                        $packageArray['email_content'][0] = ['lang' => 'en_us','html' => $email_content];
            ...
            //Load Pricing Model
            Loader::loadModels($this, ['Packages','PackageOptions','Pricings']);
    ...
    public function getPackageFields($vars = null)
    {
        ...
        $thesslstore_product_code->attach($fields->fieldSelect(
            'meta[thesslstore_product_code]',
            $product_codes,
            ($vars->meta['thesslstore_product_code'] ?? null),
            ['id' => 'thesslstore_product_code','onchange' => 'javascript:get_ssl_meta(this.value)']
        ));
        $fields->setField($thesslstore_product_code);
        unset($thesslstore_product_code);

        $field_thesslstore_vendor_name = $fields->fieldHidden('meta[thesslstore_vendor_name]', ($vars->meta['thesslstore_vendor_name'] ?? null), ['id' => 'thesslstore_vendor_name']);
        $fields->setField($field_thesslstore_vendor_name);
        unset($field_thesslstore_vendor_name);

        $field_thesslstore_is_code_signing = $fields->fieldHidden('meta[thesslstore_is_code_signing]', ($vars->meta['thesslstore_is_code_signing'] ?? null), ['id' => 'thesslstore_is_code_signing']);
        $fields->setField($field_thesslstore_is_code_signing);
        unset($field_thesslstore_is_code_signing);

        $field_thesslstore_min_san = $fields->fieldHidden('meta[thesslstore_min_san]', ($vars->meta['thesslstore_min_san'] ?? null), ['id' => 'thesslstore_min_san']);
        $fields->setField($field_thesslstore_min_san);
        unset($field_thesslstore_min_san);

        $field_thesslstore_is_scan_product = $fields->fieldHidden('meta[thesslstore_is_scan_product]', ($vars->meta['thesslstore_is_scan_product'] ?? null), ['id' => 'thesslstore_is_scan_product']);
        $fields->setField($field_thesslstore_is_scan_product);
        unset($field_thesslstore_is_scan_product);

        $field_thesslstore_validation_type = $fields->fieldHidden('meta[thesslstore_validation_type]', ($vars->meta['thesslstore_validation_type'] ?? null), ['id' => 'thesslstore_validation_type']);
    ...
    public function addService($package, array $vars = null, $parent_package = null, $parent_service = null, $status = 'pending')
    {
        ...
        if ($vars['use_module'] == 'true') {
            ...
            $additional_server = ($vars['configoptions']['additional_server'] ?? 0);
            ...
            $additional_san = ($vars['configoptions']['additional_san'] ?? 0);
            ...
            if (!empty($result->TheSSLStoreOrderID)) {
                ...
                $this->Input->setErrors(['api' => ['internal' => 'No OrderID']]);
                return;
            }
        }
    ...
    public function cancelService($package, $service, $parent_package = null, $parent_service = null)
    {
        if ($service->status == 'active') {
            ...
            $errorMessage = !$refundRes->AuthResponse->Message[0] ? $refundRes->AuthResponse->Message : $refundRes->AuthResponse->Message[0];
            if ($refundRes != null && $refundRes->AuthResponse->isError == false) {
                ...
                $this->Input->setErrors(['invalid_action' => ['internal' => $errorMessage]]);
                return;
            }
        }return null;
    }
    ...
    public function getClientTabs($package)
    {
        ...
        return [
            'tabClientCertDetails' => ['name' => Language::_('ThesslstoreModule.tab_CertDetails', true),'icon' => 'fas fa-bars'],
            'tabClientGenerateCert' => ['name' => Language::_('ThesslstoreModule.tab_GenerateCert', true),'icon' => 'fas fa-cogs'],
            'tabClientDownloadCertificate' => ['name' => Language::_('ThesslstoreModule.tab_DownloadCertificate', true), 'icon' => 'fas fa-certificate'],
            'tabClientDownloadAuthFile' => ['name' => Language::_('ThesslstoreModule.tab_DownloadAuthFile', true),'icon' => 'fas fa-file'],
            'tabClientChangeApproverEmail' => ['name' => Language::_('ThesslstoreModule.tab_ChangeApproverEmail', true),'icon' => 'fas fa-exchange-alt'],
            'tabClientResendApproverEmail' => ['name' => Language::_('ThesslstoreModule.tab_ResendApproverEmail', true),'icon' => 'fas fa-sync-alt'],
            'tabClientReissueCert' => ['name' => Language::_('ThesslstoreModule.tab_ReissueCert', true),'icon' => 'fas fa-redo-alt']
        ];
    }
    ...
    public function tabClientCertDetails($package, $service, array $get = null, array $post = null, array $files = null)
    {
        ...
        if ($service->status == 'active') {
            ...
            $this->Input->setErrors(['invalid_action' => ['internal' => Language::_('ThesslstoreModule.!error.invalid_service_status', true)]]);
            return;
        }
    }
    private function validateGenerateCertStep1($package, $vars, $san_count)
    {
        ...
        $rules = [
            'thesslstore_csr' => [
                ...
                'valid' => [
                    'rule' => [[$this, 'validateCSR'], $package->meta->thesslstore_product_code, true],
                    'message' => Language::_('ThesslstoreModule.!error.thesslstore_csr.valid', true)
                ]
            ],
            'thesslstore_webserver_type' => [
                'empty' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
            ...
            'thesslstore_signature_algorithm' => [
                'empty' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
        ...
        if ($san_count > 0) {
            ...
            $rules['thesslstore_additional_san'] = [
                'valid' => [
                    'rule' => [[$this, 'validateAdditionalSAN']],
    ...
    public function tabClientGenerateCert($package, $service, array $get = null, array $post = null, array $files = null)
    {
        ...
        if ($service->status == 'active') {
            ...
            if ($order_resp && $order_resp->OrderStatus->MajorStatus == 'Initial') {
                ...
                //Pre-filled data
                $auth_method = $post['thesslstore_auth_method'] ?? 'EMAIL';
                $admin_first_name = $post['thesslstore_admin_first_name'] ?? $client_data->first_name;
                $admin_last_name = $post['thesslstore_admin_last_name'] ?? $client_data->last_name;
                $admin_title = $post['thesslstore_admin_title'] ?? '';
                $admin_email = $post['thesslstore_admin_email'] ?? $client_data->email;
                $admin_phone = $post['thesslstore_admin_phone'] ?? $contact_number;

                $admin_org_name = $post['thesslstore_org_name'] ?? $client_data->company;
                $admin_org_division = $post['thesslstore_org_division'] ?? '';
                $admin_address1 = $post['thesslstore_admin_address1'] ?? $client_data->address1;
                $admin_address2 = $post['thesslstore_admin_address2'] ?? $client_data->address2;
                $admin_city = $post['thesslstore_admin_city'] ?? $client_data->city;
                $admin_state = $post['thesslstore_admin_state'] ?? $client_data->state;
                $admin_country = $post['thesslstore_admin_country'] ?? $client_data->country;
                $admin_zip_code = $post['thesslstore_admin_zip'] ?? $client_data->zip;

                $tech_first_name = $post['thesslstore_tech_first_name'] ?? $client_data->first_name;
                $tech_last_name = $post['thesslstore_tech_last_name'] ?? $client_data->last_name;
                $tech_title = $post['thesslstore_tech_title'] ?? '';
                $tech_email = $post['thesslstore_tech_email'] ?? $client_data->email;
                $tech_phone = $post['thesslstore_tech_phone'] ?? $contact_number;
                ...
                if ($step == 1) {
                    ...
                    if ($step == 1) {
                        ...
                        $this->view->set('thesslstore_signature_algorithms', ['SHA2-256' => 'SHA-2', 'SHA1' => 'SHA-1']);
                ...
                $this->Input->setErrors(['invalid_action' => ['internal' => Language::_('ThesslstoreModule.!error.generate_cert_invalid_certificate_status', true)]]);
                return;
            }
        } else {
            $this->Input->setErrors(['invalid_action' => ['internal' => Language::_('ThesslstoreModule.!error.invalid_service_status', true)]]);
    ...
    private function placeFullOrder($package, $service, $post, $order_status_resp)
    {
        ...
        $web_server_type = $vars->thesslstore_webserver_type ?? 'Other';
        ...
        $auth_method = $vars->thesslstore_auth_method ?? '';
        $admin_first_name = $vars->thesslstore_admin_first_name ?? '';
        $admin_last_name = $vars->thesslstore_admin_last_name ?? '';
        $admin_title = $vars->thesslstore_admin_title ?? '';
        $admin_email = $vars->thesslstore_admin_email ?? '';
        $admin_phone = $vars->thesslstore_admin_phone ?? '';
        $org_name = $vars->thesslstore_org_name ?? '';
        $org_division = $vars->thesslstore_org_division ?? '';
        $admin_address1 = $vars->thesslstore_admin_address1 ?? '';
        $admin_address2 = $vars->thesslstore_admin_address2 ?? '';
        $admin_city = $vars->thesslstore_admin_city ?? '';
        $admin_state = $vars->thesslstore_admin_state ?? '';
        $admin_country = $vars->thesslstore_admin_country ?? '';
        $admin_zip = $vars->thesslstore_admin_zip ?? '';
        $tech_first_name = $vars->thesslstore_tech_first_name ?? '';
        $tech_last_name = $vars->thesslstore_tech_last_name ?? '';
        $tech_title = $vars->thesslstore_tech_title ?? '';
        $tech_email = $vars->thesslstore_tech_email ?? '';
        $tech_phone = $vars->thesslstore_tech_phone ?? '';
        ...
        $clean_domain_name = str_replace(['www.','*.'], '', $domain_name);
        ...
        if ($service->status == 'active') {
            ...
            if ($order_resp != null && $order_resp->AuthResponse->isError == false) {
                if ($order_resp->OrderStatus->MajorStatus == 'Active' && $order_resp->OrderStatus->MinorStatus != 'PENDING_REISSUE') {
                    ...
                    $step = $post['thesslstore_reissue_step'] ?? 1;
                    ...
                    if ($vendor_name == 'CERTUM' || $is_code_signing == 'y' || $is_scan_product == 'y') {
                        ...
                        if (isset($post['thesslstore_reissue_submit'])) {
                            ...
                            $thesslstore_additional_san = $post['thesslstore_additional_san'] ?? [];
                    ...
                    $this->Input->setErrors(['invalid_action' => ['internal' => Language::_('ThesslstoreModule.!error.reissue_cert_invalid_certificate_status', true)]]);
                    return;
                }
            }
        } else {
    ...
    private function reIssueCertificate($package, $service, $post, $order_status_resp)
    {
        ...
        $vendor_name = $package->meta->thesslstore_vendor_name;
        $signature_algorithm = $vars->thesslstore_signature_algorithm;
        $additional_san = $vars->thesslstore_additional_san ?? [];
        $dns_names = explode(',', $order_status_resp->DNSNames);
        $add_san_old_new_pair = [];
        $delete_san_old_new_pair = [];
        $csr = $vars->thesslstore_csr;
        ...
        if ($service->status == 'active') {
            ...
            if (
                ...
                if ($VendorName != 'SYMANTEC' || ($VendorName == 'SYMANTEC' && $hide_changeapprover_option != 'YES' )) {
                    ...
                    $approverEmailsListArray = array_filter(array_diff($getApproverEmailsList, ['support_preprod@geotrust.com', 'support@geotrust.com']));
                    ...
                    $dnsCount = $productCode == 'quicksslpremiummd' || ($dnsNamesArray[0] == '' && $dnsNamesArray[0] == null) ? 0 : count($dnsNamesArray);
                    $domainNames = [];
                    $approverEmails = [];
                    $approverEmailsListArrays = [];
                    for ($i = 0; $i < $dnsCount; $i++) {
                        ...
                        $approverEmailsListArrays[] = array_filter(array_diff($getApproverEmailsLists, ['support_preprod@geotrust.com', 'support@geotrust.com']));
                    ...
                    $this->Input->setErrors(['invalid_action' => ['internal' => Language::_('ThesslstoreModule.!error.change_approver_email_not_available_for_product', true)]]);
                    return;
                }
            } else {
                $this->Input->setErrors(['invalid_action' => ['internal' => Language::_('ThesslstoreModule.!error.change_approver_email_not_available_for_order', true)]]);
                return;
            }
        } else {
            $this->Input->setErrors(['invalid_action' => ['internal' => Language::_('ThesslstoreModule.!error.invalid_service_status', true)]]);
    ...
    private function getApproverEmailsList($productCode, $domainName)
    {
        ...
        $emails = array_filter(array_diff($emails, ['support_preprod@geotrust.com','support@geotrust.com']));
        return $emails;
    }
        ...
        if ($service->status == 'active') {
            ...
            if (($order_resp->OrderStatus->MajorStatus == 'Pending' || $order_resp->OrderStatus->MinorStatus == 'PENDING_REISSUE')) {
                if (!$fileName && !$fileContent) {
                    ...
                    $this->Input->setErrors(['invalid_action' => ['internal' => Language::_('ThesslstoreModule.!error.download_authfile_not_available', true)]]);
                    return;
                    exit;
                } else {
                    ob_end_clean();
                ...
                $this->Input->setErrors(['invalid_action' => ['internal' => Language::_('ThesslstoreModule.!error.download_authfile_invalid_state', true)]]);
                return;
            }
            return 'success';
        } else {
        ...
        if ($service->status == 'active') {
            ...
            if ($order_resp && $order_resp->OrderStatus->MajorStatus == 'Active') {
                if (!$downloadResp->AuthResponse->isError) {
                    ...
                    $this->Input->setErrors(['invalid_action' => ['internal' => $errors]]);
                    return;
                }
            } else {
                $this->Input->setErrors(['invalid_action' => ['internal' => Language::_('ThesslstoreModule.!error.download_cert_invalid_state', true)]]);
                return;
            }
            return 'success';
        } else {
    ...
    public function getEmailTags()
    {
        return [
            'module' => [],
            'package' => [],
            'service' => ['thesslstore_order_id', 'thesslstore_token']
        ];
    }
    ...
    {
        Loader::loadModels($this, ['Clients', 'Contacts', 'Emails', 'ModuleManager']);
        ...
        $this->Emails->send('service_creation', $package->company_id, $language_code, $client->email, $tags, null, null, null, ['to_client_id' => $client->id]);
    }
    ...
    private function parseResponse($response, $ignore_error = false)
    {
        ...
        if (empty($response)) {
            ...
            if (!$ignore_error) {
                $this->Input->setErrors(['api' => ['internal' => Language::_('ThesslstoreModule.!error.api.internal', true)]]);
            }
        } elseif ($response) {
            $auth_response = null;
            ...
            if ($auth_response && property_exists($auth_response, 'isError') && $auth_response->isError) {
                ...
                if (!$ignore_error) {
                    $this->Input->setErrors(['api' => ['internal' => $error_message]]);
                }
            } elseif ($auth_response === null) {
                $success = false;

                if (!$ignore_error) {
                    $this->Input->setErrors(['api' => ['internal' => Language::_('TheSSLStore.!error.api.internal', true)]]);
                }
            }
        }
