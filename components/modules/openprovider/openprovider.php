...
class Openprovider extends RegistrarModule
{
    ...
    public function cancelService($package, $service, $parent_package = null, $parent_service = null)
    {
        ...
        $update_auto_renew_domain_response = $api->call(
            'modifyDomainRequest',
            [
                'id' => $op_domain['id'],
                'autorenew' => 'off'
        ...
        if ($update_auto_renew_domain_response->getCode() != 0) {
            $this->assignError($update_auto_renew_domain_response->getMessage());
        }

        return;
    }
    private function getOpDomain($service, $view = null)
    {
        ...
        if ($this->checkIfDomainDoesNotExistInSearchDomainResponse($domain_response)) {
            $this->assignError($this->isPageOnClientSide($view ?? 'admin')
        ...
        return $domain_response->getData()['results'][0];
    }
    ...
    private function manageNameservers(
        ...
        $op_domain = $this->getOpDomain($service, $view);
        if (!$op_domain) {
            return false;
        }
        ...
        if (!empty($post)) {
            ...
            foreach ($handles as $key => $value) {
                ...
                foreach ($domain_contacts_from_post[$key] as $key_contact => $value_contact) {
                    $domain_contacts_from_post[$key][substr(
                        $key_contact,
                        strpos($key_contact, $key) + strlen($key) + 1
                    )] = $value_contact;
    ...
    protected function getPrices(array $filters = [])
    {
        ...
        foreach ($tlds as $tld_details) {
            ...
            foreach ($currencies as $currency) {
                ...
                if (
                    isset($filters['currencies']) && !in_array($currency->code, $filters['currencies'])
        ...
        return [
            ...
            'name' => [
                ...
                'initials'   => mb_substr(
                    ...
                    0,
                    1
                ) . '.' . mb_substr($customer_array['last_name'], 0, 1) ?? null,
            ],
