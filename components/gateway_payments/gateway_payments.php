 ...
 *
 * @package blesta
 * @subpackage components.gatewaypayments
 * @copyright Copyright (c) 2010, Phillips Data, Inc.
 * @license http://www.blesta.com/license/ The Blesta License Agreement
 * @link http://www.blesta.com/ Blesta
 */
class GatewayPayments extends Component
{
    /**
    ...
    public function __construct()
    {
        ...
        Loader::loadComponents($this, ['Gateways', 'Input', 'Session']);
    ...
    public function getBuildProcess(
        ...
        if ($gateway_id !== null) {
            ...
            $gateways = $this->GatewayManager->getAllInstalledNonmerchant(
                $this->company_id,
                $currency,
     ...
     * @param bool $staff_id The ID of the staff member for which to fetch this form
     ...
     */
    public function getBuildCcForm(
        $currency,
        $client_id = null,
        ...
        if ($gateway) {
            ...
            if ($gateway_obj instanceof MerchantCcForm) {
                if ($staff_id) {
                    $gateway_obj->setStaffId($staff_id);
                }
     ...
     */
    public function getBuildAchForm(
        $currency,
        $account_info = null,
        $client_id = null,
        $client_group_id = null,
     ...
     * @param array $vars An array of bank account info used for verification purposes
     * @param int $client_id The ID of the client to fetch only the gateway available for this client
     * @param int $client_group_id The ID of the client group to fetch only the gateway available for this client group
     * @param int $gateway_id The gateway ID to verify is installed and active for the given currency
     * @return string Custom ACH form HTML from the merchant
     */
    public function getBuildAchVerificationForm(
        $currency,
        $vars = null,
        $client_id = null,
        $client_group_id = null,
        ...
        if (($errors = $this->Transactions->errors())) {
            ...
            if (isset($response['invoices']) && is_array($response['invoices'])) {
                ...
                foreach ($response['invoices'] as &$invoice) {
                    if (isset($invoice['id']) && !isset($invoice['invoice_id'])) {
    ...
    public function processPayment(
        ...
        if (empty($account_info)) {
            ...
            // If reference ID exists, the account is stored off-site (e.g., CIM, vault)
            // and must be processed using stored payment account methods
        ...
        // Use the account's gateway_id if available, otherwise use the default for the currency
        $gateway_id_to_use = (isset($account) && $account->gateway_id) ? $account->gateway_id : null;
        $gateway = $this->GatewayManager->getInstalledMerchant(
            $this->company_id,
            $currency,
            $gateway_id_to_use,
    ...
    public function capturePayment($client_id, $transaction_id, $amount = null, ?array $options = null)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            // Check if a temporary account in the current session is being used for capturing the payment
            if (!$account && !$stored) {
                $account = (object) $this->Session->read('capture_' . $transaction->type . '_account');
                $this->Session->clear('capture_' . $transaction->type . '_account');
            }

            // Parse account information
            $account_info = $this->accountToAccountInfo($account, $transaction->type);
            ...
            if (($errors = $this->Transactions->errors())) {
                ...
                if ($options['email_receipt'] === true || $options['email_receipt'] == 'true') {
                    ...
                    $tags = [
                        ...
                        'response' => $response,
                        'stored' => $stored ?? false,
                        'last_four' => ($account_info['last4'] ?? null),
                    ];

                    // Set credit card specific tags
                    if ($transaction->type == 'cc' && isset($account->type)) {
                        $card_types = $this->Accounts->getCcTypes();
                        $tags['card_type'] = $card_types[$account_info['type']] ?? null;
                    }
    ...
    private function accountToAccountInfo($account, $type)
    {
        ...
        switch ($type) {
            case 'ach':
                $account_info = [
                    'first_name' => isset($account->first_name) ? $account->first_name : null,
                    'last_name' => isset($account->last_name) ? $account->last_name : null,
