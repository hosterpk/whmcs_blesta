...
class Ccavenue extends NonmerchantGateway
{
    ...
    public function buildProcess(array $contact_info, $amount, array $invoice_amounts = null, array $options = null)
    {
        ...
        $encRequest = $this->encrypt($merchant_data, $encryption_key);
    ...
    /**
     * Encrypts the given value using 128-bit AES in CBC mode
     *
    ...
    public function encrypt($value, $key)
    {
        $key = hex2bin(md5($key));
        $iv = pack(
            "C*", 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f
        );
    ...
    /**
     * Decrypts the given value using 128-bit AES in CBC mode
     *
    ...
    function decrypt($value, $key)
    {
        ...
        $data = openssl_decrypt(hex2bin($value), 'AES-128-CBC', $key, OPENSSL_RAW_DATA, $iv);
    ...
    public function validate(array $get, array $post)
    {
        ...
        if ($enc_response) {
            ...
            $rec_string = $this->decrypt($enc_response, $encryption_key);
            $all_params = [];
