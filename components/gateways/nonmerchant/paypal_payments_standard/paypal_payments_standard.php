...
class PaypalPaymentsStandard extends NonmerchantGateway
{
                ...
                'valid'=>[
                    ...
                    'rule'=>function ($map) {
                        ...
                        foreach ($map as $entry) {
                            ...
                            // Check if the entry has two parts
                            if (count($entry) !== 2) {
                                return false;
                            }

                            // Check if the email address is valid
                            if (!filter_var($entry[0] ?? '', FILTER_VALIDATE_EMAIL)) {
                                return false;
                            }

                            // Check if the client ID is a number
                            if (!is_numeric($entry[1] ?? null)) {
                                return false;
                            }
                        }
                    ...
                    'message'=>Language::_('PaypalPaymentsStandard.!error.payment_mapping.valid', true)
                ],
                'duplicated'=>[
                    'if_set'=>true,
                    'rule'=>function ($map) {
                        ...
                        foreach ($map as $entry) {
                            ...
                            // Check if the email address isn't duplicated
                            if (in_array($entry[0], $existing_emails)) {
                                return false;
                            }

                            if (!empty($entry[0])) {
                                $existing_emails[] = $entry[0];
                            }
                        }

                        return true;
                    },
                    'message'=>Language::_('PaypalPaymentsStandard.!error.payment_mapping.duplicated', true)
                ]
            ]
        ];
    ...
    public function validate(array $get, array $post)
    {
        ...
        // If available, fetch the client_id from the URL
        $client_id = ($get['client_id'] ?? null);

        // If the URL doesn't contain the client_id, check if the payer_email is associated to a client_id
        $map = $this->getClientsMap();
        if (!$client_id && array_key_exists(($post['payer_email'] ?? null), $map)) {
            $client_id = $map[$post['payer_email']];
        }

        // If the payer_email is not assigned to a client_id on the mapping data, try to match the payer_email to a client
        if (!$client_id) {
            $client_id = $this->clientIdFromEmail(($post['payer_email'] ?? null));
        }

        // Ensure we are not processing again the same transaction
        if (!isset($this->Transactions)) {
            Loader::loadModels($this, ['Transactions']);
        }
        $transaction = $this->Transactions->getByTransactionId(($post['txn_id'] ?? null), $client_id);
        if (!empty($transaction) && $transaction->status == 'approved') {
            return;
        }
    ...
    /**
     * Fetches the client mapping data
     *
     * @return array An array where the key represents the email address of the PayPal account used by the client
     *  and the value represents the client_id
     */
    private function getClientsMap()
    {
        $clients = [];

        if (!empty($this->meta['payment_mapping'])) {
            $map = explode("\n", trim($this->meta['payment_mapping']));
            foreach ($map as $entry) {
                $entry = explode(',', $entry, 2);
