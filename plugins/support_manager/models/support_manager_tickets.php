...
class SupportManagerTickets extends SupportManagerModel
{
    ...
    public function add(array $vars, $require_email = false)
    {
        ...
        // Trigger the SupportManager.addTicketBefore event
        extract($this->triggerEvent(
            'addTicketBefore',
            ['vars' => $vars, 'require_email' => $require_email]
        ));
        ...
        if (isset($vars['client_id']) && $vars['client_id'] == '') {
            $vars['client_id'] = null;
        }
        if (isset($vars['recipients']) && !empty($vars['recipients'])) {
            $vars['recipients'] = array_values($vars['recipients']);
        }
        if (isset($vars['contacts']) && !empty($vars['contacts'])) {
            $vars['contacts'] = array_values($vars['contacts']);
        }

        // Set the first recipient as the ticket email if a client ID hasn't been provided
        if (empty($vars['client_id']) && empty($vars['email']) && !empty($vars['recipients']) && is_array($vars['recipients'])) {
            $vars['email'] = reset($vars['recipients']);
        }

        $vars['date_updated'] = $vars['date_added'] = date('c');

        // Set request_feedback based on department settings
        if (!isset($vars['request_feedback']) && isset($vars['department_id'])) {
            Loader::loadModels($this, ['SupportManager.SupportManagerDepartments']);
            $department = $this->SupportManagerDepartments->get($vars['department_id']);
            $vars['request_feedback'] = ($department && !empty($department->feedback_ticket_interval)) ? 1 : 0;
        }
        ...
        if ($this->Input->validates($vars)) {
            ...
            $fields = ['code', 'department_id', 'staff_id', 'service_id', 'client_id',
                'email', 'summary', 'priority', 'request_feedback', 'rating', 'rating_comment', 'status', 'date_added', 'date_updated'];
            ...
            extract($this->triggerEvent(
                'addTicketAfter',
                ['vars' => $vars, 'require_email' => $require_email, 'ticket_id' => $ticket_id]
            ));
    ...
    private function addCustomFields(int $ticket_id, array $custom_fields)
    {
        if (!empty($custom_fields)) {
            foreach ($custom_fields as $field_id => $value) {
                ...
                if ($field->type == 'emergency' && ($value ?? '0') == '1') {
                    $this->Record->where('id', '=', $ticket_id)->update('support_tickets', ['priority' => 'emergency']);
                }
    ...
    public function edit($ticket_id, array $vars, $log = true)
    {
        ...
        // Trigger the SupportManager.editTicketBefore event
        extract($this->triggerEvent(
            'editTicketBefore',
            ['vars' => $vars, 'ticket_id' => $ticket_id]
        ));
        ...
        // Update the ticket
        if ($this->Input->validates($vars)) {
            $fields = ['department_id', 'staff_id', 'service_id', 'client_id', 'email', 'summary',
                'priority', 'request_feedback', 'rating', 'rating_comment', 'status', 'date_updated'];
            ...
            // Set the first recipient as the ticket email if a client ID hasn't been provided
            if (empty($vars['client_id']) && !empty($vars['recipients']) && is_array($vars['recipients'])) {
                $vars['email'] = reset($vars['recipients']);
            }
            ...
            // Set ticket recipients
            if (isset($vars['recipients'])) {
                $this->setRecipients($ticket_id, $vars['recipients']);
            }

            // Set ticket contacts
            if (isset($vars['contacts'])) {
                $this->setContacts($ticket_id, $vars['contacts']);
            }
            ...
            // Trigger the SupportManager.editTicketAfter event
            extract($this->triggerEvent(
                'editTicketAfter',
                ['vars' => $vars, 'ticket_id' => $ticket_id]
            ));
     ...
     *
     * @param int $ticket_id The ID of the ticket to set the recipients
     * @param array $recipients An array containing the email addresses of the recipients
     */
    private function setRecipients($ticket_id, array $recipients)
    {
        // Remove all existing recipients
        $this->Record->from('support_ticket_recipients')
            ->where('support_ticket_recipients.ticket_id', '=', $ticket_id)
            ->where('support_ticket_recipients.contact_id', '=', null)
            ->delete();
        ...
        foreach ($recipients as $recipient) {
            ...
            $recipient = ['ticket_id' => $ticket_id, 'email' => $recipient];
            $this->Record->insert('support_ticket_recipients', $recipient, $fields);
        }
    }
     ...
     *
     * @param int $ticket_id The ID of the ticket to set the contacts
     * @param array $contacts An array containing IDs of the contacts
     */
    private function setContacts($ticket_id, array $contacts)
    {
        // Remove all existing contacts
        $this->Record->from('support_ticket_recipients')
            ->where('support_ticket_recipients.ticket_id', '=', $ticket_id)
            ->where('support_ticket_recipients.email', '=', null)
            ->delete();

        // Add new recipients
        $fields = ['ticket_id', 'contact_id'];
        foreach ($contacts as $contact) {
            if (empty($contact)) {
                continue;
            }

            $contact = ['ticket_id' => $ticket_id, 'contact_id' => $contact];
            $this->Record->insert('support_ticket_recipients', $contact, $fields);
        }
    }
     ...
     */
    public function sendFeedbackReminder(array $ticket_ids)
    {
        ...
        if (!empty($ticket_ids)) {
            foreach ($ticket_ids as $ticket_id) {
                ...
                $email_action = 'SupportManager.ticket_feedback';
                ...
                $tags = [
                    'ticket' => $ticket,
                    'client' => $client,
                    'feedback_ticket_url' => $this->getFeedbackTicketUrl($ticket->id, $hostname)
                ];
                $options = [
                ...
                // Set feedback reminder as sent
                $this->Record->where('id', '=', $ticket->id)
                    ->update('support_tickets', ['request_feedback' => 0]);
            }
        }
    }
    ...
    public function sendReminder(array $ticket_ids)
    {
        ...
        if (!empty($ticket_ids)) {
            foreach ($ticket_ids as $ticket_id) {
                ...
                // Skip closed/trash tickets as a safety guard
                if (in_array($ticket->status, ['closed', 'trash'])) {
                    continue;
                }
     ...
     */
    public function notifyFeedbackAllByDepartment($department_id)
    {
        Loader::loadModels($this, ['Companies', 'SupportManager.SupportManagerDepartments']);

        $department = $this->SupportManagerDepartments->get($department_id);
        if ($department && !empty($department->feedback_ticket_interval)) {
            $reminder_cutoff = $this->dateToUtc($this->Date->modify(
                date('c'),
                '-' . abs($department->feedback_ticket_interval) . ' minutes',
                'c',
                Configure::get('Blesta.company_timezone')
            ));
            ...
            $this->Record->select('support_tickets.id')
                ->from('support_tickets')
                ->where('support_tickets.department_id', '=', $department->id)
                ->where('support_tickets.date_closed', '<=', $reminder_cutoff)
                ->where('support_tickets.status', '=', 'closed')
                ->where('support_tickets.rating', '=', null)
                ->where('support_tickets.request_feedback', '=', 1);
            ...
            $this->sendFeedbackReminder($notify_tickets);
        }
    }
    ...
    public function notifyAllByDepartment($department_id)
    {
        ...
        if ($department && $department->reminder_ticket_interval !== null) {
            ...
            // Always exclude closed/trash regardless of configured filter
            $this->Record->where('support_tickets.status', 'not in', ['closed', 'trash']);
            if (!empty($statuses)) {
    ...
    public function addReply($ticket_id, array $vars, array $files = null, $new_ticket = false)
    {
        ...
        // Trigger the SupportManager.addReplyBefore event
        extract($this->triggerEvent(
            'addReplyBefore',
            ['vars' => $vars, 'ticket_id' => $ticket_id, 'files' => $files, 'new_ticket' => $new_ticket]
        ));

        // Truncate reply
        if (isset($vars['details'])) {
            $vars['details'] = mb_substr($vars['details'], 0, 16777215);
            while (strlen($vars['details']) > 16777215) {
                // Shorten the string character by character until it's within the byte limit
                $vars['details'] = mb_substr($vars['details'], 0, -1);
            }
        }
        ...
        $loggable_fields = ['department_id' => 'department_id', 'service_id' => 'service_id', 'client_id' => 'client_id',
            'ticket_staff_id' => 'staff_id', 'summary' => 'summary',
            'priority' => 'priority', 'status' => 'status'];

        $loggable_field_set = false;
        foreach ($loggable_fields as $loggable_field) {
            $loggable_field_set = $loggable_field_set || isset($vars[$loggable_field]);
        }
        $ticket = null;
        if (!$new_ticket && $loggable_field_set) {
     ...
     * @return void
     */
    public function setFeedback($ticket_id, array $vars)
    {
        $ticket = $this->get($ticket_id);
    ...
    public function get($ticket_id, $get_replies = true, array $reply_types = null, $staff_id = null)
    {
        ...
        if ($ticket) {
            ...
            $ticket->recipients = $this->Record->select('support_ticket_recipients.email')
                ->from('support_ticket_recipients')
                ->where('support_ticket_recipients.ticket_id', '=', $ticket->id)
                ->where('support_ticket_recipients.contact_id', '=', null)
                ->fetchAll();
            $ticket->recipients = $this->Form->collapseObjectArray($ticket->recipients, 'email');

            $ticket->contacts = $this->Record->select('support_ticket_recipients.contact_id')
                ->from('support_ticket_recipients')
                ->where('support_ticket_recipients.ticket_id', '=', $ticket->id)
                ->where('support_ticket_recipients.email', '=', null)
                ->fetchAll();
            $ticket->contacts = $this->Form->collapseObjectArray($ticket->contacts, 'contact_id');
        }
    ...
    public function getContactEmails($ticket_id)
    {
        ...
        // Fetch the email addresses of the recipients set on the ticket
        $recipients = $this->Record->select('support_ticket_recipients.email')->
            from('support_ticket_recipients')->
            where('support_ticket_recipients.ticket_id', '=', $ticket_id)->
            where('support_ticket_recipients.contact_id', '=', null)->
            fetchAll();
        ...
        // Fetch the email addresses of the contacts set on the ticket
        if (!empty($ticket->contacts)) {
            $contacts = $this->Record->select(['contacts.email'])->
                from('support_ticket_recipients')->
                innerJoin('contacts', 'contacts.id', '=', 'support_ticket_recipients.contact_id', false)->
                where('support_ticket_recipients.ticket_id', '=', $ticket_id)->
                where('support_ticket_recipients.email', '=', null)->
                group(['contacts.email'])->
                fetchAll();
        } else {
            // Fetch the email addresses of the contacts that replied to this ticket
            $contacts = $this->Record->select(['contacts.email'])->
                from('support_replies')->
     ...
     * @param string $hostname The company hostname to link
     */
    private function getFeedbackTicketUrl($ticket_id, $hostname)
    {
        $key = mt_rand();
        $hash = $this->generateReplyHash($ticket_id, $key);

        $url = $hostname . $this->getWebDirectory() . Configure::get('Route.client')
            . '/plugin/support_manager/client_tickets/feedback/' . $ticket_id
            . '/?sid=' . rawurlencode($this->systemEncrypt('h=' . substr($hash, -16) . '|k=' . $key));
    ...
    private function getReplyRules(array &$vars, $new_ticket = false)
    {
        ...
        if (isset($vars['recipients'])) {
            foreach ($vars['recipients'] as $key => $recipient) {
                if (empty($recipient)) {
                    unset($vars['recipients'][$key]);
                }
            }
        }
        ...
        if (isset($vars['contacts'])) {
            foreach ($vars['contacts'] as $key => $contact) {
                if (empty($contact)) {
                    unset($vars['contacts'][$key]);
                }
            }
        }
        ...
        $rules = [
            ...
            'recipients[]' => [
                'duplicated' => [
                    'if_set' => true,
                    'rule' => function ($recipient) use ($vars) {
                        $count = array_count_values($vars['recipients'] ?? []);

                        return ($count[$recipient] ?? 0) > 1;
                    },
                    'negate' => true,
                    'message' => $this->_('SupportManagerTickets.!error.recipients.duplicated')
                ],
                'valid' => [
                    'pre_format' => [function($recipient) {return $recipient ?? '';}, 'trim'],
                    'if_set' => true,
                    'rule' => [[$this, 'validateEmail']],
                    'message' => $this->_('SupportManagerTickets.!error.recipients.valid')
                ]
            ],
            'contacts[]' => [
                'valid' => [
                    'pre_format' => [function($contact) {return $contact ?? null;}],
                    'if_set' => true,
                    'rule' => [[$this, 'validateExists'], 'id', 'contacts'],
                    'message' => $this->_('SupportManagerTickets.!error.contacts.valid')
                ]
            ],
            'type' => [
                'format' => [
    ...
    private function getRules(array &$vars, $edit = false, $require_email = false)
    {
            ...
            'priority' => [
                'valid' => [
                    ...
                    'rule' => function ($priority) use ($vars) {
                        $parent = new stdClass();
                        Loader::loadModels($parent, ['SupportManager.SupportManagerDepartments']);
                        Loader::loadComponents($parent, ['Session']);

                        $priorities = [];
                        $staff_id = $parent->Session->read('blesta_staff_id');
                        if ($staff_id) {
                            // Staff members can use any priority regardless of the department settings
                            $priorities = array_keys($this->getPriorities());
                        } else {
                            $department = $parent->SupportManagerDepartments->get($vars['department_id']);
                    ...
                    'message' => $this->_('SupportManagerTickets.!error.priority.valid')
                ],
                'format' => [
                    'if_set' => true,
        ...
        // Set rules for required custom fields
        if (isset($vars['department_id']) && $this->validateExists($vars['department_id'], 'id', 'support_departments')) {
            $fields = $this->Record->select()
                ->from('support_department_fields')
                ->where('support_department_fields.department_id', '=', $vars['department_id'])
                ->where('support_department_fields.required', '=', 1)
                ->fetchAll();

            foreach ($fields as $field) {
                $rules['custom_fields[' . $field->id . ']'] = [
                    'empty' => [
                        'if_set' => true,
                        'rule' => 'isEmpty',
                        'negate' => true,
                        'message' => $this->_('SupportManagerTickets.!error.custom_fields.empty', true, $field->label)
                    ]
                ];
            }
        }

        // The client ID is required if there are no recipients and if an email hasn't been defined
        if (empty($vars['recipients']) && empty($vars['email']) && !$edit) {
            unset($rules['client_id']['exists']['if_set']);
        }

        // Remove client_id rule if at least one recipient has been provided
        if (!empty($vars['recipients']) && is_array($vars['recipients'])) {
