...
class SupportManagerKbCategories extends SupportManagerModel
{
    ...
    {
        // Load models
        Loader::loadModels($this, ['SupportManager.SupportManagerKbArticles']);
        ...
        if ($this->Input->validates($vars)) {
            ...
            // Update articles access
            if (($vars['update_articles'] ?? '0') == '1') {
                $articles = $this->Record->select('support_kb_articles.*')
                    ->innerJoin(
                ...
                // Include sub-articles
                if (($vars['update_recursive'] ?? '0') == '1') {
                    // Fetch subcategories
                    $sub_categories = $this->getAll($vars['company_id'], $category_id, true);

                    // Update access for all categories
                    $sub_categories_ids = [];
                    foreach ($sub_categories as $sub_category) {
                        $this->edit($sub_category->id, [
                            'access' => $vars['access'],
                            'update_articles' => $vars['update_articles'],
                            'update_recursive' => $vars['update_recursive']
                        ]);
                        $sub_categories_ids[] = $sub_category->id;
                    }

                    // Fetch all the sub-articles from all the sub-categories
                    if (!empty($sub_categories_ids)) {
                        $sub_articles = $this->Record->select('support_kb_articles.*')
                            ->innerJoin(
                                'support_kb_article_categories',
                                'support_kb_article_categories.article_id',
                                '=',
                            ...
                            ->where('support_kb_article_categories.category_id', 'in', $sub_categories_ids)
                            ->from('support_kb_articles')
                            ->fetchAll();
                        $articles = array_merge($articles, $sub_articles);
                    }
                }

                foreach ($articles as $article) {
                    $this->SupportManagerKbArticles->edit($article->id, ['access' => $vars['access']]);
                }
            }
        ...
        $rules = [
            ...
            'update_articles' => [
                'format' => [
                    'if_set' => true,
                    'rule' => ['in_array', ['0', '1']],
                    'message' => $this->_('SupportManagerKbCategories.!error.update_articles.format')
                ]
            ],
            'update_recursive' => [
                'format' => [
                    'if_set' => true,
                    'rule' => ['in_array', ['0', '1']],
                    'message' => $this->_('SupportManagerKbCategories.!error.update_recursive.format')
                ]
            ]
        ];
