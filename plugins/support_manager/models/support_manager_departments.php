...
class SupportManagerDepartments extends SupportManagerModel
{
     ...
     *  - feedback_ticket_interval The number of minutes since the ticket has been closed at
     *      which point a feedback notification should be sent (optional, default null for never)
    ...
    public function add(array $vars)
    {
        ...
        // Trigger the SupportManager.addDepartmentBefore event
        ...
        if ($this->Input->validates($vars)) {
            $fields = [
                ...
                'default_priority', 'host', 'user', 'pass', 'port', 'security', 'box_name',
                'mark_messages', 'oauth2_provider', 'oauth2_user', 'oauth2_client_id',
                'oauth2_client_secret', 'oauth2_access_token', 'oauth2_code', 'oauth2_refresh_token',
                'clients_only', 'enable_related_services', 'require_captcha', 'override_from_email',
                'send_ticket_received', 'close_ticket_interval', 'delete_ticket_interval', 'feedback_ticket_interval',
            ...
            if ($department_id) {
                ...
                $this->addPriorities($department_id, $vars['priorities'] ?? []);
            }

            // Trigger the SupportManager.addDepartmentAfter event
            extract($this->triggerEvent(
                'addDepartmentAfter',
                ['vars' => $vars, 'department_id' => $department_id]
            ));
    ...
    private function addFields(int $department_id, array $fields)
    {
        $accepted_fields = [
            ...
            'min', 'max', 'step', 'client_add', 'required', 'encrypted', 'auto_delete', 'options'
        ];

        // Format fields
        if (!empty($fields['label'])) {
            foreach ($fields['label'] as $i => $label) {
                // Emergency fields must always be set to client only
                if (isset($fields['type'][$i]) && $fields['type'][$i] == 'emergency') {
                    $fields['visibility'][$i] = 'client_only';
                }

                // If the visibility is set to Client Only, it must be added by the client
                if (isset($fields['visibility'][$i]) && $fields['visibility'][$i] == 'client_only') {
                    $fields['client_add'][$i] = '1';
                }
     ...
     *
     * @param int $department_id The ID of the department where the priorities must be added
     * @param array $priorities A list ticket priorities available for this department
     */
    private function addPriorities(int $department_id, array $priorities)
    {
        $accepted_fields = ['department_id', 'priority'];
        ...
        if (!empty($priorities)) {
            foreach ($priorities as $priority) {
                ...
                $this->Record->insert('support_department_priorities', $vars, $accepted_fields);
            }
        }
    }
    ...
    public function edit($department_id, array $vars)
    {
        ...
        // Trigger the SupportManager.editDepartmentBefore event
        extract($this->triggerEvent(
            'editDepartmentBefore',
            ['vars' => $vars, 'department_id' => $department_id]
        ));
        ...
        if ($this->Input->validates($vars)) {
            ...
            $this->editPriorities($department_id, $vars['priorities'] ?? []);

            // If feedback reminders were enabled or disabled, update open tickets
            if ($department->feedback_ticket_interval == null && $vars['feedback_ticket_interval'] != null) {
                $this->updateTicketFeedbackReminderFlag($department->id, true);
            } elseif ($department->feedback_ticket_interval != null && $vars['feedback_ticket_interval'] == null) {
                $this->updateTicketFeedbackReminderFlag($department->id, false);
            }

            // Trigger the SupportManager.editDepartmentAfter event
            extract($this->triggerEvent(
                'editDepartmentAfter',
                ['vars' => $vars, 'department_id' => $department_id]
            ));
     ...
     *
     * @param type $department_id The department for which to update tickets
     * @param type $request_feedback Whether or not to request feedback for the tickets
     */
    private function updateTicketFeedbackReminderFlag($department_id, $request_feedback) {
        $this->Record->where('department_id', '=', $department_id)->
            where('status', '!=', 'closed')->
            where('status', '!=', 'trash')->
            update('support_tickets', ['request_feedback' => $request_feedback ? 1 : 0]);
    }
     ...
     *
     * @param int $department_id The ID of the department where the priorities must be updated
     * @param array $priorities A list ticket priorities available for this department
     */
    private function editPriorities(int $department_id, array $priorities)
    {
        $this->Record->from('support_department_priorities')
            ->where('support_department_priorities.department_id', '=', $department_id)
            ->delete();

        $this->addPriorities($department_id, $priorities);
    }
    ...
    public function get($department_id)
    {
        ...
        if ($department) {
            ...
            // Get priorities
            $department->priorities = $this->Form->collapseObjectArray(
                $this->Record->select()
                    ->from('support_department_priorities')
                    ->where('support_department_priorities.department_id', '=', $department_id)
                    ->fetchAll(),
                'priority'
            );
    ...
    public function getMethods()
    {
        return [
            ...
            'oauth2' => $this->_('SupportManagerDepartments.methods.oauth2')
        ];
    }
     ...
     *
     * @return array A list of providers and their language
     */
    public function getProviders()
    {
        return [
            'google' => $this->_('SupportManagerDepartments.providers.google'),
            'microsoft' => $this->_('SupportManagerDepartments.providers.microsoft')
        ];
    }
    ...
    public function getPriorities(?int $department_id = null)
    {
        ...
        if (($department = $this->get($department_id))) {
            foreach ($department->priorities as $priority) {
                $priorities[$priority] = $this->_('SupportManagerDepartments.priorities.' . $priority);
            }
        } else {
            $priorities = [
    ...
    public function getFieldTypes()
    {
        return [
            ...
            'emergency' => $this->_('SupportManagerDepartments.field_types.emergency'),
    ...
    public function getVisibilityOptions()
    {
        return [
            ...
            'client_only' => $this->_('SupportManagerDepartments.visibility_options.client_only'),
    ...
    private function getRules(array $vars, $edit = false)
    {
        $rules = [
            ...
            'default_priority' => [
                'format' => [
                    'if_set' => true,
                    'rule' => ['in_array', ($vars['priorities'] ?? array_keys($this->getPriorities()))],
            ...
            'priorities[]' => [
                'empty' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => $this->_('SupportManagerDepartments.!error.priorities[].empty')
                ],
                'format' => [
                    'rule' => ['in_array', array_keys($this->getPriorities())],
                    'message' => $this->_('SupportManagerDepartments.!error.priorities[].format')
                ]
            ],
            'host' => [
                'format' => [
            ...
            'fields[required][]' => [
                'format' => [
                    'if_set' => true,
                    'rule' => ['in_array', [0,1]],
                    'message' => $this->_('SupportManagerDepartments.!error.required.format')
                ]
            ],
            'fields[encrypted][]' => [
                'format' => [
     ...
     *
     * @param string $method The email handling method ('none', 'pipe', 'imap', 'pop3', 'oauth2')
