...
{
    // Try to get the subject from parser first
    $value = $email->getHeader('Subject');

    // Fallback to lowercase header key if empty
    if (empty($value) && isset($email->parts[1]['headers']['subject'])) {
        $value = $email->parts[1]['headers']['subject'];
    }

    // Decode MIME encoded header if needed
