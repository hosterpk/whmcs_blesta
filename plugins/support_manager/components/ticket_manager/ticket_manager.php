 ...
 *
 * @package blesta
 * @subpackage plugins.supportmanager.components
...
class TicketManager
{
     ...
     */
    public function processDepartmentEmails($departments = null)
    {
        $company_id = Configure::get('Blesta.company_id');

        Loader::load(PLUGINDIR . 'support_manager' . DS . 'vendors' . DS . 'autoload.php');
        Loader::load(PLUGINDIR . 'support_manager' . DS . 'vendors' . DS . 'php-imap' . DS . 'Imap2Mailbox.php');
        ...
        Loader::loadModels($this, ['SupportManager.SupportManagerDepartments', 'Email']);

        // Fetch departments
        if (!is_array($departments) || empty($departments)) {
            $departments = $this->SupportManagerDepartments->getByMethod($company_id, ['pop3', 'imap', 'oauth2']);
        }
        ...
        foreach ($departments as $department) {
            ...
            if ($service == 'oauth2') {
                ...
                $token = $transport->setUsername($department->oauth2_user)
                    ->setClientId($department->oauth2_client_id)
                    ->setClientSecret($department->oauth2_client_secret)
                    ->setProvider($department->oauth2_provider)
                    ->setRefreshToken($department->oauth2_refresh_token)
                    ->refreshAccessToken(false, false);

                $password = !empty($token->access_token) ? $token->access_token : $department->oauth2_access_token;

                // Update refresh token
                if (!empty($token->refresh_token)) {
                    $data = ['oauth2_refresh_token' => $token->refresh_token];
                    $this->SupportManagerDepartments->edit($department->id, $data);
                }

                // For OAuth2, use the oauth2_user (email) instead of the department user field
                $user = $department->oauth2_user;

                // Connect to IMAP server
                $flags = [];
                $flags[] = 'imap';  // OAuth2 uses IMAP protocol, not 'oauth2'
                if ($security) {
                    $flags[] = $security;
                }
                ...
                $connection = '{' . $host . ($port ? ':' . $port : null) . '/'
                    . implode('/', $flags) . '}' . imap2_utf7_encode($box_name);

                try {
                    $mailbox = new Imap2Mailbox($connection, $user, $password);
                    ...
                    foreach ($mailbox->searchMailbox($search_type) as $mail_id) {
                        ...
                        $header = $mailbox->fetchHeader($mail_id);
                        $body = $mailbox->fetchBody($mail_id);

                        $email->setText($header . $body);
    ...
    public function processTicketFromEmail(MimeMailParser $email, $department = null)
    {
        ...
        $cc = array_filter($cc, function($recipient) use ($to) {
            return !in_array($recipient, $to);
        });
        $cc = array_filter($cc, function($recipient) use ($from) {
            return $recipient !== $from;
        });
        $cc = array_unique($cc);

        // Perform email address validation against the blacklist
        if (!$this->Blacklist->verify($from, 'email')) {
        ...
        if ($ticket_info) {
            ...
            foreach ($to as $address) {
                ...
                if ($department) {
                    ...
                    // Add CC recipients to the ticket
                    if (!empty($cc)) {
                        $ticket_info['recipients'] = array_values($cc);
                    }
