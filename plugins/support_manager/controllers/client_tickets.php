...
class ClientTickets extends SupportManagerController
{
    ...
    public function preAction()
    {
        ...
        if (!in_array($this->action, ['add', 'reply', 'close', 'departments', 'getpriorities', 'getattachment', 'feedback'])) {
            $this->requireLogin($this->base_uri . 'plugin/support_manager/client_tickets/departments/');
        }

        $this->uses(['SupportManager.SupportManagerTickets', 'SupportManager.SupportManagerDepartments', 'Blacklist']);
    ...
    public function add()
    {
        $this->uses(['Services', 'ModuleManager', 'Clients']);
        ...
        if (!empty($this->post)) {
            ...
            // Perform email address validation against the blacklist
            if (!$this->Blacklist->verify($this->post['email'] ?? '', 'email')) {
                $this->Session->clear();
                $captcha = null;
                $success = false;
                $errors = [
                    'email' => ['blacklist' => Language::_('ClientTickets.!error.email.blacklist', true)]
                ];
            }
        ...
        $service_ids = [];
        if ($this->client_id) {
            $service_ids = $this->fetchClientServicesIds($this->client_id);
        }

        // Get contacts for logged-in clients
        $contacts = $this->Form->collapseObjectArray(
            $this->Contacts->getAll($this->client_id, 'billing') + $this->Contacts->getAll($this->client_id, 'other'),
    ...
    public function feedback()
    {
        ...
        // Give feedback to the ticket
        if (!empty($this->post)) {
            // Create a transaction
            $this->SupportManagerTickets->begin();

            // Set ticket feedback
            $this->SupportManagerTickets->setFeedback($ticket->id, $this->post);
            ...
            if (($errors = $this->SupportManagerTickets->errors())) {
                ...
                $this->flashMessage(
                    'message',
                    Language::_('ClientTickets.!success.ticket_feedback_updated', true, $ticket->code),
                    null,
                    false
                );
                ...
                $this->redirect(
                    ...
                    . ($access['allow_reply_by_url']
                        ? 'feedback/' . $ticket->id . '/?sid=' . rawurlencode($access['sid'])
                        : ''
                    )
                );
            }
    ...
    public function reply()
    {
        ...
        $this->uses(['SupportManager.SupportManagerStaff', 'Services', 'ModuleManager', 'Contacts']);
        ...
        $service = $this->Services->get($ticket->service_id ?? null);
        $module = $this->ModuleManager->get($service->package->module_id ?? null);
        ...
        foreach ($fields as $field) {
            ...
            switch ($field->type) {
                case 'text':
                    ...
                    $input_fields->setField(
                        $custom_fields->attach(
                            $input_fields->fieldText(
                                ...
                                $vars[$field->id] ?? '',
                                array_merge(['id' => 'custom_fields_' . $field->id], $disabled, $required),
                    ...
                    break;
                case 'quantity':
                    $custom_fields = $input_fields->label($field->label, 'custom_fields_' . $field->id, array_merge(['class' => 'd-block'], $disabled));
                    $input_fields->setField(
                        $custom_fields->attach(
                            $input_fields->fieldText(
                                'custom_fields[' . $field->id . ']',
                                ...
                                $input_fields->label($field->description, null, array_merge(['class' => 'd-block'], $disabled))
                            )
                        )
                    );
                    break;
                    ...
                    foreach ($options['options'] ?? [] as $value => $name) {
                        $custom_fields->attach(
                            $input_fields->fieldRadio(
                                ...
                                array_merge(['id' => 'custom_fields_' . $field->id . '_' . $value], $disabled, $required),
                    ...
                    $input_fields->setField(
                        $custom_fields->attach(
                            $input_fields->fieldCheckbox(
                                ...
                                '1',
                                ($vars[$field->id] ?? '0') == '1',
                                array_merge(['id' => 'custom_fields_' . $field->id, 'class' => 'emergency'], $disabled, $required),
                                $input_fields->label($field->label, 'custom_fields_' . $field->id, $disabled)
                            )
                        )
                    );
