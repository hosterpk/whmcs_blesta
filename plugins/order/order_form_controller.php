...
class OrderFormController extends OrderController
{
    ...
    private function getLineTotals($client_id, &$cart, $recurring = false)
    {
        ...
        $currency = $this->Currencies->get($this->SessionCart->getData('currency'), $this->company_id);
        ...
        foreach ($total_keys as $total_key) {
            if (isset($line_totals[$total_key])) {
                $line_totals[$total_key] = [
                    'amount' => round($line_totals[$total_key], $currency->precision ?? 4),
                    'amount_formatted' => $this->CurrencyFormat->format($line_totals[$total_key], $currency->code)
                ];
            }
        }
        ...
        foreach ($line_totals['tax'] as &$tax) {
            ...
            $tax = [
                'amount' => round($tax, $currency->precision ?? 4),
                'amount_formatted' => $this->CurrencyFormat->format($tax, $currency->code)
            ];
        }
    ...
    protected function isValidItem($item)
    {
        ...
        // Validate with the order type
        if (method_exists($this->order_type, 'validateItem') && !$this->order_type->validateItem($item)) {
            return false;
        }
    ...
    protected function getPaymentOptions($currency = null)
    {
        ...
        $merchant_gateway = $this->GatewayManager->getInstalledMerchant(
            ...
            null,
            $this->client->id ?? null,
            $this->order_form->client_group_id ?? null
        );
    ...
    protected function runAntifraudCheck(array $order_settings, stdClass $client)
    {
        ...
        $antifraud = ($order_settings['antifraud'] ?? null);

        // Trigger the Order.fraudCheckBefore event
        extract($this->PluginManager->executeAndParseEvent(
            'fraudCheckBefore',
            ['order_settings' => $order_settings, 'client' => $client]
        ));
        ...
        try {
            ...
            // Trigger the Order.fraudCheckAfter event
            extract($this->PluginManager->executeAndParseEvent(
                'fraudCheckAfter',
                [
                    'status' => $status,
