...
class Signup extends OrderFormController
{
    ...
    public function index()
    {
        ...
        $this->uses(['Users', 'Contacts', 'Countries', 'States', 'ClientGroups', 'EmailVerifications', 'Blacklist']);
        ...
        $this->SessionCart->setData('tos_accepted', false);
        ...
        if (!empty($this->post)) {
            ...
            if (!$this->Blacklist->verify($this->post['email'], 'email')) {
                ...
                $success = false;
                $errors = [
                    'email' => ['blacklist' => Language::_('Signup.!error.email.blacklist', true)]
                ];
            }
            ...
            if (
                $this->order_form->type == 'registration'
                && $this->order_form->require_tos
                && !$this->SessionCart->getData('tos_accepted', false)
            ) {
                $errors = ['agree_tos' => ['invalid' => Language::_('Signup.!error.invalid_agree_tos', true)]];
            }
                ...
                if (
                    ...
                    if (empty($email_verification)) {
                        $this->EmailVerifications->add([
                            ...
                            'redirect_url' => ($this->order_form->type == 'registration')
                                ? $this->base_uri . 'client/'
        ...
        // Check if we require TOS agreement before registering
        $show_tos = false;
        if (($this->order_form->type ?? '') == 'registration' && $this->order_form->require_tos == '1') {
            $show_tos = true;
        }
