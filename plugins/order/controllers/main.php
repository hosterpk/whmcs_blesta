...
class Main extends OrderFormController
{
    ...
    public function packages()
    {
        ...
        $pricing_id = null;
        $package_id = null;
        $preset_pricing = false;

        // Set pricing and package ID from the url
            ...
            foreach ($packages as $package) {
                ...
                if (!$sold_out && !$limit_reached && isset($package->pricing[0])) {
                    if ($pricing_id === null) {
                        ...
                        // Set the pricing id to the default pricing for the given package, if not
                        // default back to the first pricing available
                        $pricing_id = $package->pricing[0]->id;
                        foreach ($package->pricing as $pricing) {
                            if (($pricing->default ?? '0') == '1') {
