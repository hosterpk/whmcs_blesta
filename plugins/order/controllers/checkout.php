...
class Checkout extends OrderFormController
{
    ...
    private function verifyClientAndOrder()
    {
        ...
        if (!$this->client) {
            $this->Session->write('blesta_forward_to', $this->base_uri . 'order/checkout/index/' . $this->order_form->label);
     ...
     * @param bool $redirect Whether or not to redirect to the complete page, true by default
     * @return stdClass An object representing the invoice for the current order after applying credits
     */
    private function applyCreditsToOrder($order, $invoice, $redirect = true)
    {
        ...
        if (isset($this->post['apply_credit']) && $this->post['apply_credit'] == 'true') {
            ...
            if ($amount_applied !== false) {
                ...
                if ($invoice->due <= 0 && $invoice->date_closed !== null && $redirect) {
                    $this->redirect($this->base_uri . 'order/checkout/complete/' . $this->order_form->label . '/' .
                        $order->order_number . '/');
                }
            }
        }
    ...
    public function getPaymentConfirmation()
    {
        ...
        if ($invoice->due <= 0 && $invoice->date_closed !== null) {
            $transaction_response['redirect'] = $this->base_uri . 'order/checkout/complete/' . $this->order_form->label . '/' .
                $order->order_number . '/';
        }
