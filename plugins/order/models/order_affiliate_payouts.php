...
class OrderAffiliatePayouts extends OrderModel
{
    ...
    {
        // Trigger the Order.addPayoutBefore event
        extract($this->triggerEvent('addPayoutBefore', ['vars' => $vars]));
        ...
        if ($this->Input->validates($vars)) {
            ...
            $payout_id = $this->Record->lastInsertId();
    ...
    public function edit($payout_id, array $vars)
    {
        ...
        extract($this->triggerEvent('editPayoutBefore', ['payout_id' => $payout_id, 'vars' => $vars]));
        ...
        if ($this->Input->validates($vars)) {
            ...
            if (isset($vars['status']) && $vars['status'] !== $payout->status) {
                if ($payout->status == 'approved' && $vars['status'] == 'declined') {
                    // Refund the amount if we decline a previously approved payout
