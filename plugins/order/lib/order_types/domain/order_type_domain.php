 ...
 *
 * @package blesta
 * @subpackage plugins.order.lib.order_types
...
class OrderTypeDomain extends OrderType
{
    ...
    /**
     * Ensures that all the data submitted for the item is valid
     *
     * @param array $item An array containing item details to validate
    ...
    public function handleRequest(array $get = null, array $post = null, array $files = null)
    {
        ...
        // Fetch spotlight TLDs early so they can be used to limit keyword searches
        ...
        if (isset($post['domain'])) {
            ...
            // Determine if this is a keyword search or full domain search
            if ($this->isKeywordSearch($post['domain'])) {
                // For keyword searches, use the keyword as SLD and search spotlight TLDs only
                // (or first 4 TLDs if no spotlights are configured)
                $sld = $post['domain'];
                if (empty($post['tlds'])) {
                    $post['tlds'] = !empty($spotlight_tlds)
                        ? array_keys($spotlight_tlds)
                        : array_slice(array_keys($tlds), 0, 4);
                }

                $offset = isset($post['offset']) ? (int) $post['offset'] : 0;
                $tlds_batch = array_slice($post['tlds'], $offset, Configure::get('Blesta.results_per_page'));
            } else {
                // Full domain search
                ...
                $tlds_batch = $post['tlds'];
            }

            // Build domains array for availability checking
            $domains_by_module = [];
            foreach ($tlds_batch as $tld) {
                $domain_name = $sld . $tld;
                $pack = $this->domainPackageGroup($domain_name, $tlds);
                if ($pack) {
                    $pack[0] = $this->updatePackagePricing($pack[0], $currency);

                    $domains[$domain_name] = new stdClass();
                    $domains[$domain_name]->package = $pack[0];
                    $domains[$domain_name]->group = $pack[1];

                    // Group domains by module for bulk checking
                    $module_key = $pack[0]->module_id . '_' . $pack[0]->module_row;
                    if (!isset($domains_by_module[$module_key])) {
                        $domains_by_module[$module_key] = [
                            'module_id' => $pack[0]->module_id,
                            'module_row' => $pack[0]->module_row,
                            'domains' => []
                        ];
                    }
                    $domains_by_module[$module_key]['domains'][] = $domain_name;
                }
            ...
            if (empty($domains)) {
                ...
                // Check availability using bulk method when possible
                $availability = [];
                foreach ($domains_by_module as $module_info) {
                    $module_domains = $module_info['domains'];
                    $method_name = isset($post['transfer']) ? 'checkTransferAvailability' : 'checkAvailability';
                    $bulk_method_name = isset($post['transfer']) ? 'checkTransferAvailability' : 'bulkCheckAvailability';

                    // Try to use bulk method first
                    if (method_exists($this->ModuleManager->get($module_info['module_id']), $bulk_method_name)) {
                        $bulk_results = $this->ModuleManager->moduleRpc(
                            $module_info['module_id'],
                            $bulk_method_name,
                            [$module_domains],
                            $module_info['module_row']
                        );

                        if (is_array($bulk_results)) {
                            foreach ($bulk_results as $domain => $result) {
                                $availability[$domain] = $result;
                                if (isset($post['transfer']) && is_null($availability[$domain])) {
                                    $availability[$domain] = true;
                                }
                            }
                        } else {
                            // Fallback to individual checks if bulk method fails
                            foreach ($module_domains as $domain) {
                        ...
                        // Fallback to individual checks for modules without bulk support
                        foreach ($module_domains as $domain) {
                            $availability[$domain] = $this->ModuleManager->moduleRpc(
                                $module_info['module_id'],
                                $method_name,
        ...
        foreach ($domains as $domain => $pack) {
            foreach ($pack->package->pricing as $key => $pricing) {
                if (isset($post['transfer'])) {
                    ...
                    if (is_null($domains[$domain]->package->pricing[$key]->price_transfer)) {
                        unset($domains[$domain]->package->pricing[$key]);
                    }
                }
            }

            if (empty($pack->package->pricing)) {
                unset($domains[$domain]);
            }
        }

        if (empty($domains) && isset($post['transfer'])) {
            $this->Input->setErrors([
                'domain' => [
                    'invalid' => Language::_('OrderTypeDomain.!error.domain.invalid', true)
                ]
        ...
        // Get domains package group for spotlight TLD display logic
        ...
        if ($get['getAllTlds'] ?? false) {
            ...
            if ($this->isAjax()) {
                echo json_encode(['html' => $this->view->fetch('lookup')]);
                exit();
            } else {
    ...
    /**
     * Determines if the input is a keyword search (no TLD) or full domain search
     *
     * @param string $domain The domain/keyword input
     * @return bool True if this appears to be a keyword search, false for full domain
     */
    private function isKeywordSearch($domain)
    {
        // If the input contains no dots, it's likely a keyword
        if (strpos($domain, '.') === false) {
            return true;
        }

        // If it contains a dot but doesn't appear to be a valid TLD, treat as keyword
        $tld = $this->getTld($domain);
        if (empty($tld) || $tld === $domain) {
            return true;
        }
