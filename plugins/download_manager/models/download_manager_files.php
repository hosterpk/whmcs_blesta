 ...
 *
 * @package blesta
 * @subpackage plugins.downloadmanager.models
...
{
    /**
     * @var array A list of directories that are allowed for reading
     */
    ...
    public function __construct()
    {
        ...
        if (isset($uploads_dir['value'])) {
            $this->allowed_paths[] = realpath($uploads_dir['value']);
        }
        if (isset($temp_dir['value'])) {
            $this->allowed_paths[] = realpath($temp_dir['value']);
        }
    ...
    public function add(array $vars, array $files)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            // Verify the upload file is from an allowed directory
            $allowed = false;
            foreach ($this->allowed_paths as $path) {
                if (str_contains(realpath($vars['file_name']), $path)) {
                    $allowed = true;
                }
            }
    ...
    public function delete($file_id)
    {
        ...
        if ($file) {
            ...
            $allowed = false;
            foreach ($this->allowed_paths as $path) {
                if (str_contains(realpath($file->file_name), $path)) {
                    $allowed = true;
                }
            }
    ...
    public function validateFile($file_name, array $files)
    {
        ...
        if ($file_name !== null) {
            // Verify the file is from an allowed directory
            $allowed = false;
            foreach ($this->allowed_paths as $path) {
                if (str_contains(realpath($file_name), $path)) {
                    $allowed = true;
                }
            }
