 ...
 *
 * @package blesta
 * @subpackage plugins.systemstatus
...
class AdminMain extends SystemStatusController
{
    ...
    public function index()
    {
        ...
        $this->uses(['Companies', 'License', 'Logs', 'Settings']);
        ...
        if (array_key_exists('updates', (array)$license_data) && $license_data['updates'] !== false) {
            $expired = false;
        ...
        // Check if the upload directory is within the root web directory
        $uploads_dir = $this->SettingsCollection->fetchSetting($this->Companies, $this->company_id, 'uploads_dir');
        if (strpos(realpath($uploads_dir['value']), realpath(ROOTWEBDIR)) !== false) {
            $errors->upload_dir = [
                'icon' => $icons['warning'],
                'message' => Language::_('AdminMain.index.upload_dir', true),
                'link' => $this->base_uri . 'settings/system/general/basic/',
                'link_text' => Language::_('AdminMain.index.upload_dir_configure', true),
                'status_value' => $this->status_values['upload_dir']['serious']
            ];
        }
