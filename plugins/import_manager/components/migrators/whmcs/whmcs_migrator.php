...
class WhmcsMigrator extends Migrator
{
    ...
    protected function importClients()
    {
        ...
        foreach ($clients as $client) {
            ...
            $vars = [
                ...
                'status' => strtolower($client->status) == 'active' ? 'active' : 'inactive'
            ];
        ...
        // Import client users
        if (method_exists($this->WhmcsClients, 'getClientUsers')) {
            $client_users = $this->fetchall ? $this->WhmcsClients->getClientUsers()->fetchAll() : $this->WhmcsClients->getClientUsers();
            foreach ($client_users as $client_user) {
                // Create client, if does not exists
                if (empty($this->mappings['clients'][$client_user->id])) {
                    $user_id = null;
                    try {
                        $user_id = $this->createUser([
                            'username' => $this->decode($client_user->email),
                            'password' => substr($client_user->password, 0, 64),
                            'date_added' => $this->getValidDate($client_user->datecreated)
                        ]);
                    } catch (Throwable $e) {
                        $this->logException($e);
                    }
                    ...
                    $vars = [
                        'id_format' => '{num}',
                        'id_value' => $client_user->id,
                        'user_id' => $user_id,
                        'client_group_id' => $this->mappings['client_groups'][$client_user->groupid],
                        'status' => strtolower($client_user->status) == 'closed' ? 'inactive' : 'active'
                    ];
                    $this->local->insert('clients', $vars);
                    $client_id = $this->local->lastInsertId();

                    $this->mappings['clients'][$client_user->id] = $client_id;
                }

                // Create user for contact
                $contact_user_id = null;
                try {
                    $contact_user_id = $this->createUser([
                        'username' => $this->decode($client_user->user_email),
                        'password' => substr($client_user->password, 0, 64),
                        'date_added' => $this->getValidDate($client_user->datecreated)
                    ]);
                } catch (Throwable $e) {
                ...
                // Create contact
                $vars = [
                    'client_id' => $this->mappings['clients'][$client_user->id],
                    'user_id' => $contact_user_id,
                    'contact_type' => 'billing',
                    'first_name' => $this->decode($client_user->firstname),
                    'last_name' => $this->decode($client_user->lastname),
                    'company' => $this->decode($client_user->companyname != '' ? $client_user->companyname : null),
                    'email' => $this->decode($client_user->email),
                    'address1' => $this->decode($client_user->address1 != '' ? $client_user->address1 : null),
                    'address2' => $this->decode($client_user->address2 != '' ? $client_user->address2 : null),
                    'city' => $this->decode($client_user->city != '' ? $client_user->city : null),
                    'state' => $this->getValidState(
                        $client_user->country,
                        $this->decode($client_user->state != '' ? $client_user->state : null)
                    ),
                    'zip' => $this->decode($client_user->postcode != '' ? $client_user->postcode : null),
                    'country' => $client_user->country != '' ? $client_user->country : $this->default_country,
                ...
                $remote_permissions = explode(',', $client_user->permissions);
                $mapped_permissions = [
                    'contacts' => 'client_contacts',
                    'products' => 'client_services',
                    'domains' => 'domains.*',
                    'invoices' => 'client_invoices',
                    'tickets' => 'support_manager.*',
                    'emails' => 'client_emails',
                    'orders' => 'order.*',
                    'quotes' => 'client_quotations'
                ];

                foreach ($remote_permissions as $permission) {
                    $permission = trim($permission);
                    if (array_key_exists($permission, $mapped_permissions)) {
                        $vars = [
                            'contact_id' => $contact_id,
                            'client_id' => $this->mappings['clients'][$client_user->id],
                            'area' => $mapped_permissions[$permission] ?? '.*'
                        ];
                        $this->local->insert('contact_permissions', $vars);
                    }
                }
            }
        }
    ...
    protected function importInvoices()
    {
        ...
        foreach ($lines as $line) {
            ...
            if ($line->type == 'Hosting') {
                $service_id = $this->mappings['services'][$line->relid] ?? null;
            }
            if ($line->type == 'DomainRegister') {
                $service_id = $this->mappings['domains'][$line->relid] ?? null;
            }
    ...
    protected function importPackages()
    {
        ...
        if (!$this->PluginManager->isInstalled('domains', Configure::get('Blesta.company_id'))) {
            $this->PluginManager->add(['dir' => 'domains', 'company_id' => Configure::get('Blesta.company_id')]);
        }
        ...
        foreach ($tlds as $tld) {
            ...
            if (
                ...
                $this->mappings['tld'][$tld->extension] = $this->local->lastInsertId();
            }
    ...
    protected function importDomains()
    {
        ...
        foreach ($domains as $domain) {
            ...
            if (!isset($this->mappings['packages'][$tld . $domain->registrar])) {
                ...
                $id_value = ($this->mappings['tld_last_id'] ?? 0) + $i++;
                $vars = [
                    'id_format' => '{num}',
                    'id_value' => $id_value,
            ...
            $this->local->insert('services', $vars);
            $service_id = $this->local->lastInsertId();
            $this->mappings['domains'][$domain->id] = $service_id;
            ...
            $this->addServiceFields((array)$domain, $mapping, true);
        }
        $this->local->commit();
        unset($domains);
    }
    ...
    protected function importSupportTickets()
    {
        ...
        foreach ($responses as $response) {
            ...
            $vars = [
                ...
                'name' => strlen($name) > 64 ? substr($name, 0, 60) . '...' : $name,
     ...
     */
    private function addServiceFields($service, $mapping, $domain = false)
    {
        ...
        foreach ($mapping['service_fields'] as $key => $field) {
            ...
            $vars = [
                'service_id' => $this->mappings[($domain ? 'domains' : 'services')][$service['id']],
                'key' => $field->key,
    ...
    /**
     * Maps WHMCS language names to Blesta language codes
     *
     * @param string $whmcs_language The WHMCS language name (e.g., "spanish", "hebrew")
     * @return string The Blesta language code (e.g., "es_es", "he_il")
     */
    protected function mapKbLanguage($whmcs_language)
    {
        if (empty($whmcs_language)) {
            return 'en_us';
        }
    ...
    {
        Loader::loadModels($this, ['Companies']);
        $this->loadModel('WhmcsKnowledgeBase');

        // Import categories
        $categories = $this->fetchall
            ? $this->WhmcsKnowledgeBase->getCategories()->fetchAll()
            : $this->WhmcsKnowledgeBase->getCategories();

        foreach ($categories as $category) {
            try {
                $vars = [
                    'parent_id' => !empty($category->parentid) ? $this->mappings['kb_categories'][$category->parentid] ?? null : null,
                    'company_id' => Configure::get('Blesta.company_id'),
                    'name' => $this->decode($category->name),
                    'description' => $this->decode($category->description),
                    'access' => isset($category->hidden) && $category->hidden == '1' ? 'hidden' : 'public',
                ...
                $this->mappings['kb_categories'][$category->id] = $this->local->lastInsertId();

                $this->debug('Category: ' . $category->name);
            } catch (Throwable $e) {
                $this->logException($e);
            }
        }
        ...
        $articles = $this->fetchall
            ? $this->WhmcsKnowledgeBase->getArticles()->fetchAll()
            : $this->WhmcsKnowledgeBase->getArticles();

        foreach ($articles as $article) {
            try {
                $access = 'public';
                if (isset($article->private) && $article->private == '1') {
                    $access = 'hidden';
                }
                ...
                $vars = [
                    'company_id' => Configure::get('Blesta.company_id'),
                    'access' => $access,
                    'up_votes' => isset($article->useful) ? (int)$article->useful : 0,
                    'down_votes' => isset($article->notuseful) ? (int)$article->notuseful : 0,
                    'date_created' => !empty($article->created_at)
                        ? $this->Companies->dateToUtc($article->created_at)
                        : $this->Companies->dateToUtc(date('c')),
                    'date_updated' => !empty($article->updated_at)
                        ? $this->Companies->dateToUtc($article->updated_at)
                        : $this->Companies->dateToUtc(date('c'))
                ];
                $this->local->insert('support_kb_articles', $vars);
                $article_id = $this->local->lastInsertId();

                $language = $this->mapKbLanguage($article->language ?? null);

                $vars = [
                    'article_id' => $article_id,
                    'lang' => $language,
                    'title' => $this->decode($article->title),
                    'body' => $this->decode($article->article),
                    'content_type' => 'html'
                ];
                $this->local->insert('support_kb_article_content', $vars);

                $links = $this->WhmcsKnowledgeBase->getArticleLinks($article->id);
                foreach ($links as $link) {
                    if (isset($this->mappings['kb_categories'][$link->categoryid])) {
                        $vars = [
                            'category_id' => $this->mappings['kb_categories'][$link->categoryid],
                            'article_id' => $article_id
                        ];
                ...
                $this->debug('Article: ' . $article->title);
            } catch (Throwable $e) {
                $this->logException($e);
            }
        }
