...
- **Harness final run (PHP 8.3, MariaDB 10.6.27):** `result: PASS`, exit 0. Clean `1.8.0 → 1.9.0` transition (columns absent → present), backfill 0 NULL, idempotent re-run clean, fresh-install ≡ upgrade (byte-identical DDL fragment), AC1 raw-dup rejected + race fall-through returns winner + exactly one pending, release semantics (cancel frees / posted blocks). Post-run live state clean: 2 production vouchers backfilled (distinct keys), 0 leftover disposable rows, scratch dropped, 0 orphan links.
...
- **Verification:** plugin suite 165 tests / 0 failures (PHP 8.3 and 8.2); gateway suite 233 / 1 **pre-existing baseline** failure (`ambiguous/bill-payment-inquiry-empty-currency.xml`, `[[kuickpay-failclosed-empty-currency-red]]`) — disclosed as baseline, not a regression. `php -l` clean on all 8 changed PHP files on 8.3 and 8.2; `config.json` valid. Evidence: `docs/kuickpay/active-context-concurrency-verification.md`.
