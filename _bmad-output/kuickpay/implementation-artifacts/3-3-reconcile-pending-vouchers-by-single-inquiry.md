...
- 2026-06-12: Post-completion fixes after live testing. **(1) Single inquiry must key on the Consumer Number** (commit `4ab81a21`): `buildInquiryRequest` now sends the WSDL field `consumerNumber` with `voucher->consumer_number` (was `RegistrationNumber` + the registration value, which KuickPay ignores → empty result), and `buildParserContext` matches `expected_consumer_number` (KuickPay echoes the consumer number in inquiry result field `[1]`) instead of `expected_registration_number`. The inquiry uses the **separately-configured inquiry credentials** (distinct from voucher creds; gateway "Use voucher credentials for inquiries" unchecked). **(2) Lock release TypeError** (commit `12fedf62`): `KuickpayReconcileLocks::release()` called `Record->delete('kuickpay_reconcile_locks')`, but minPHP `Record::delete()` takes `?array $columns` and reads the table from `from()` — passing a string threw a `TypeError` that blank-paged the bulk run and would crash the `reconcile_pending` cron on every lock release. Corrected to `from('kuickpay_reconcile_locks')->where(...)->delete()`. The original 2026-06-10 lock-release patch missed it because the tests use a fake lock repository, not the real `Record`. **Learning:** exercise the real minPHP `Record` (not only fakes) for the lock/DB seams.
