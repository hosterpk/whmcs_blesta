...
- 2026-06-11: Post-completion installer fix (commit `333af18f`). The as-built `install()` created the `kuickpay_vouchers` unique keys via `setKey([...], 'unique')` with no explicit name; per minPHP `Record::setKey`, the name defaults to the first field, so both `(company_id, consumer_number)` and `(company_id, registration_number)` resolved to index name `company_id` and collided — the second overwrote the first, so a real plugin install would have created only the registration unique key and silently dropped the consumer-number uniqueness guard AC3/NN#3 require. Gave every key on `kuickpay_vouchers` and `kuickpay_voucher_invoices` the explicit name Task 1.3/1.4 already specified (`uniq_kuickpay_vouchers_consumer`, `uniq_kuickpay_vouchers_reg`, `idx_kuickpay_vouchers_status/_client/_txn`, `uniq_kuickpay_voucher_invoices_link`, `idx_kuickpay_voucher_invoices_inv`), closing the auto-named-index deviation Story 3-3 (Task line 52) explicitly warned against. Schema-only installer change, `php -l` clean; no migration/backfill needed — surfaced while verifying the live Blesta DB, which had zero `kuickpay%` tables and the plugin still registered at v1.0.0 (never upgraded to 1.4.0), so the corrected schema is produced on the next fresh plugin install (Uninstall → Install).
