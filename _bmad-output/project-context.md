...
- For web-facing environments, prefer external dev tooling outside the app tree instead of installing Composer dev dependencies into this checkout. Composer is available at `/root/tools/composer/composer`, and the isolated PHPUnit 8.5 runner is available at `/root/tools/phpunit-8.5/vendor/bin/phpunit`; these paths are intentionally outside `/home/hosterpk/beta.hosterpk.com`.
- KuickPay gateway tests were verified with the external PHPUnit 8.5 runner using `cd components/gateways/nonmerchant/kuickpay && /root/tools/phpunit-8.5/vendor/bin/phpunit --bootstrap tests/bootstrap.php tests`. Do not use `-c build/phpunit.xml` for this component unless the config is fixed, because PHPUnit resolves `tests/bootstrap.php` relative to `build/` and looks for missing `build/tests/bootstrap.php`.
- If external PHPUnit tooling is no longer needed on a web-facing host, remove it with `rm -rf /root/tools/phpunit-8.5`; remove Composer too with `rm -rf /root/tools/composer` if no longer needed. Do not commit these external tool directories or recreate them under the web root.
