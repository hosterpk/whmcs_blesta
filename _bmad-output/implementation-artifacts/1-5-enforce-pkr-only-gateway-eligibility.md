...
- [x] **Task 3 — PKR-first policy visibility + no hard-coded conversion (AC3)** [Source: settings.pdt:160-169; language/en_us/kuickpay.php:41-43; kuickpay.php:215-221; prd.md FR-5 line 133]
  ...
  - [x] 3.3 **Prove no hard-coded conversion** (NN#2): the guard reads `getCurrencies()`, and no exchange-rate/USD-to-PKR/multiplier constant exists in `kuickpay.php` or `lib/`. Add a Dev Note recording the grep proof (Task 6.3). No code change is expected for this sub-AC beyond the guard sourcing PKR from config.

- [x] **Task 4 — Language string for the blocked state (AC2, AC3)** [Source: language/en_us/kuickpay.php; UX-DR28; ux EXPERIENCE.md "Non-PKR Blocked" surface line 28]
  - [x] 4.1 Add the customer-facing key `Kuickpay.!error.currency_ineligible` — clean, no raw/internal detail, honest that the invoice is unaffected. Exact line to append (single-quote, one-key-per-line style):
    ```php
...
- 2026-06-09: Red phase: `/root/tools/phpunit-8.5/vendor/bin/phpunit tests/KuickPayCurrencyEligibilityTest.php` failed before implementation with undefined `currencyEligible()` in 4 helper assertions; config assertion passed.
- 2026-06-09: Green/refactor: focused eligibility test passed, then full component-local suite passed with `/root/tools/phpunit-8.5/vendor/bin/phpunit --bootstrap tests/bootstrap.php tests` → 37 tests, 151 assertions.
- 2026-06-09: Syntax: `php -l` passed for `kuickpay.php`, `language/en_us/kuickpay.php`, and `tests/KuickPayCurrencyEligibilityTest.php`.
- 2026-06-09: Static verification: guard grep confirms `currencyEligible()`, `$this->currency`, `getCurrencies()`, strict `in_array`, and `currency_ineligible`; conversion/rate grep and `PKR` business-logic grep over `kuickpay.php` + `lib/*.php` returned no hits.
- 2026-06-09: Config/scope verification: `config.json` diff is empty and decoded currencies are exactly `['PKR']`; `../tests` is missing, so root Blesta PHPUnit regression coverage was not available.

### Completion Notes List

- Added `protected currencyEligible()` using `(string) ($this->currency ?? '')` against `(array) $this->getCurrencies()` with strict `in_array`, so eligibility is sourced from gateway config and fails closed for unset, empty, non-configured, or unloaded currencies.
- Added a currency-first `buildProcess()` guard before the companion-plugin check. The ineligible path sets `Kuickpay.!error.currency_ineligible` via `Input->setErrors()` and leaves the single neutral process-view return shape intact. No voucher persistence, customer reference panel, schema, SOAP call, or posting logic was added.
- Preserved Blesta's native currency hiding contract: `config.json` remains unchanged with only `PKR`, and the gateway code records that `gateway_currencies` joins remain the primary show/hide mechanism while the `buildProcess()` guard is the fail-closed backstop.
- Confirmed AC3 without a settings view change: the existing `currency_policy` select and `currency_policy_note` expose the PKR-only policy, and `editSettings()` still restricts `currency_policy` to `pkr_only`.
- Added customer-facing localized copy for ineligible invoice currency. Copy is language-keyed and does not expose internals or diagnostics.
- Added `KuickPayCurrencyEligibilityTest` covering eligible configured currency, non-configured currency, missing/null currency, empty configured currencies fail-closed, and exact `config.json` currency declaration.
- Runtime Blesta/MySQL/manual pay-page verification was not run in this environment; acceptance is supported by source review, component-local PHPUnit, syntax checks, static greps, and config assertion.

### File List

- components/gateways/nonmerchant/kuickpay/kuickpay.php
- components/gateways/nonmerchant/kuickpay/language/en_us/kuickpay.php
- components/gateways/nonmerchant/kuickpay/tests/KuickPayCurrencyEligibilityTest.php
