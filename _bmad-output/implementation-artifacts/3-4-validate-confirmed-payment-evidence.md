...
- [ ] **Task 3 — Wire the gate into `KuickPayReconcileService::persistEvidence()` (AC: 2,8,9)**
  ...
  - [ ] **Merge the validator reason codes into the existing `diagnostic_summary` JSON** (it is already a string at line 190, built from evidence only — decode → append → re-encode):
    ```php
